#!/usr/bin/env python3
"""One-command entrypoint for the Japan Expanded 0.28.1 validation suite."""
...
from jxp_validation.government_identity import check_government_identity
from jxp_validation.ideas import check_route_ideas
from jxp_validation.imjin import check_imjin_history
from jxp_validation.internal_systems import check_internal_systems
...
def run_checks(
    ...
    companion_context = (
        ValidationContext(companion_root) if companion_root.is_dir() else None
    )
    return (
        check_metadata(context, expected_version),
        check_script_parsing(context),
        check_missions(context),
        check_daimyo_coverage(context),
        ...
        check_government_identity(context, game_root, companion_context),
        ...
        check_japanese_names(context),
        check_sprites_and_dds(context),
        check_toyotomi_history(context, game_root),
        check_imjin_history(context),
