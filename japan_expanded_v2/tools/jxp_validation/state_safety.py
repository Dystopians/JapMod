...
DISASTER_CONTRACTS = {
    ...
    "jxp_ikko_rising": DisasterContract(
        ...
        + (
            ...
            Requirement("jxp_has_any_route_trigger", "yes", True),
            Requirement("tag", "IJP", True),
        ),
        can_start=(
            Requirement("has_any_disaster", "no"),
            Requirement("is_year", "1470"),
            Requirement("num_of_cities", "8"),
            Requirement("jxp_tenka_order_at_least_50", "yes", True),
            Requirement("has_province_flag", "jxp_map_compat_ikko_heartland"),
        ),
    ),
...
def _resolved_flag_from_end(
    ...
    if on_end is None:
        return None
    if isinstance(on_end.value, Object):
        end_body = on_end.value
    elif isinstance(on_end.value, Scalar) and on_end.value.text in events:
