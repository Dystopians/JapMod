...
CORE_TAGS = {"JAP", "KJP", "CJP", "EJP", "RFJ", "SJP", "IJP", "WAK", "TOY"}
ROUTE_FLAGS = {
    "jxp_path_sakoku",
    "jxp_path_open_trade",
    "jxp_path_kirishitan",
...
class Profile:
    ...
    full_idea_groups: frozenset[str] = frozenset()
...
BASE_PROFILES = tuple(
    ...
    Profile("TOY Toyotomi realm", "TOY", "shinto", "eastern"),
    ...
    Profile(
        "JAP Buddhist (Shinbutsu)",
        "JAP",
        "shinto",
        "eastern",
        flags=frozenset({"jxp_path_buddhist"}),
    ),
    Profile(
...
def evaluate_potential(
    ...
    def evaluate_entry(entry: Entry) -> frozenset[bool]:
        ...
        if key == "ROOT":
            if not isinstance(value, Object):
                return malformed(entry, "ROOT requires an object")
            return evaluate_sequence(value)
        ...
        if key == "full_idea_group":
            return frozenset({text in profile.full_idea_groups})
        if key == "has_dlc":
        ...
        # Agent A depth overlays deliberately replace one or two common slots
        # while keeping the five-column contract.  These sets mirror the
        # schema-validated daimyo_depth_plan.json content tiers; resolving the
        # calls here lets profile simulation prove the exclusions instead of
        # treating them as unknown both-ways predicates.
        depth_s_tags = {
            "ODA", "ASK", "TKG", "IMG", "TKD", "UES", "HJO", "MRI",
            "SMZ", "OTM", "OUC", "DTE", "SOO", "HNG", "MYO", "MTS",
        }
        depth_a_tags = {
            "CSK", "ARI", "AZI", "KRD", "MOG", "MTU", "NBS", "RKK",
            "RZJ", "STM", "STO", "TGR", "UKT",
        }
        depth_all_tags = {
            ...
            "TKD", "TKG", "TKI", "TTI", "UES", "UTN", "YMN", "RKK", "KYO", "WKT",
            "TGS", "NHT", "ANK", "HNM", "MOG", "ASN", "OSK", "SMA", "DHO", "MKM",
            "SGR", "KMP", "ARI", "MTU", "STM", "HNG", "AZI", "MYO", "STO", "RZJ",
            "UKT", "TGR", "HCS", "MTS", "KRD", "YMC", "NBS",
        }
        ...
        uses_japanese_reform_track = (
            profile.culture_group == "japanese_g"
            or bool(profile.reforms & {"daimyo", "indep_daimyo", "shogunate"})
            or profile.tag in CORE_TAGS
            or bool(route_flags)
        )
        scripted_values = {
            ...
            "jxp_a_uses_depth_slot_2_trigger": profile.tag in depth_s_tags,
