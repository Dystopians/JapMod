...
def decode_eu4_special_gameplay(payload: bytes) -> str:
    """Decode BOM-free EU4SpecialEscape gameplay bytes independently.

    The independent decoder lets validation inspect the actual Chinese ruler
    names while the active file remains byte-compatible with double-byte
    language patches.
    """
    ...
    while index < len(payload):
        ...
        if marker not in markers:
            ...
            while end < len(payload) and payload[end] not in markers:
                end += 1
            output.extend(
                chr(CP1252_TO_UNICODE.get(byte, byte))
                for byte in payload[index:end]
            )
            index = end
            continue
        if index + 2 >= len(payload):
            raise ClausewitzParseError("truncated EU4SpecialEscape triplet")
        low = payload[index + 1]
...
def read_clausewitz_text(path: Path) -> str:
    ...
    if any(marker in data for marker in (b"\x10", b"\x11", b"\x12", b"\x13")):
        try:
            return decode_eu4_special_gameplay(data)
        except (ClausewitzParseError, UnicodeDecodeError, ValueError) as exc:
            raise ClausewitzParseError(
                f"{path}: cannot decode EU4SpecialEscape gameplay bytes"
            ) from exc
    try:
