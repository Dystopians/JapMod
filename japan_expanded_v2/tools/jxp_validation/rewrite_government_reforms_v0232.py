...
def route_potential(route: str, reform_id: str) -> str:
    lines = [
        "\tpotential = {",
        "\t\tOR = {",
        f"\t\t\tjxp_final_state_{route}_trigger = yes",
    ]
    if route in {"sakoku", "open"}:
        flag = ROUTE_CONDITIONS[route][1]
        foreign_flags = [
            candidate_flag
            for candidate_route, (_tag, candidate_flag) in ROUTE_CONDITIONS.items()
        ...
        lines.extend(
            [
                ...
                "\t\t\t\tNOT = {",
                "\t\t\t\t\tOR = {",
                *(f"\t\t\t\t\t\thas_country_flag = {item}" for item in foreign_flags),
                "\t\t\t\t\t}",
                "\t\t\t\t}",
                "\t\t\t}",
            ]
...
def rewrite_route_file(path: Path) -> tuple[str, ...]:
    ...
    for reform_id, start, end in reversed(top_level_blocks(text)):
        ...
        route = route_key(reform_id)
        potential = route_potential(route, reform_id)
