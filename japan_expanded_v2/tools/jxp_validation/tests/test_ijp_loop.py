...
class IjpCommonwealthLoopTests(unittest.TestCase):
    ...
    def test_rejects_production_route_without_jodo_shinshu_gate(self) -> None:
        _directory, root = self._temporary_contract()
        _mutate(root, TRIGGER_FILE, "\treligion = jodo_shinshu\n", "")
        codes = _codes(root)
        self.assertIn("ijp_loop.trigger_contract", codes)
    ...
    def test_rejects_pressure_clamp_without_upper_bound(self) -> None:
        ...
        _mutate(
            root,
            EFFECT_FILE,
            "\t\t\tset_variable = { which = jxp_a_shinshu_militancy value = 100 }\n",
            "",
        )
    ...
    def test_rejects_route_exit_that_leaves_pending_guard(self) -> None:
        ...
        _mutate(
            ...
            "\tjxp_a_ijp_clear_terminal_settlement_effect = yes\n"
    ...
    def test_rejects_ikko_disaster_without_terminal_settlement_exclusion(self) -> None:
        _directory, root = self._temporary_contract()
        _mutate(
            root,
            DISASTER_FILE,
            "\t\tNOT = { jxp_a_ijp_has_terminal_settlement_trigger = yes }\n",
            "",
        )
