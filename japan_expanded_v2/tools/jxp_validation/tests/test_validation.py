...
from jxp_validation.reforms import (
    EXPECTED_DAIMYO_STAGE_COUNT,
    EXPECTED_FINAL_STATE_POLITICAL_COUNT,
    EXPECTED_FOUNDER_COUNT,
    EXPECTED_JAPANESE_TRACK_COUNT,
    EXPECTED_LEVEL_MEMBERS,
    EXPECTED_VISIBLE_ROUTE_COUNT,
    FINAL_STATE_POLITICAL_REQUIREMENTS,
    check_reforms,
)
from jxp_validation.chinese_compat import PINNED_LAUNCHER_SUPPORTED_VERSION
...
class DescriptorCompatibilityTests(unittest.TestCase):
    def test_live_descriptors_match_pinned_launcher_version(self) -> None:
        repo_root = LIVE_MOD_ROOT.parent
        descriptors = (
            LIVE_MOD_ROOT / "descriptor.mod",
            repo_root / "japan_expanded_v2.mod",
            repo_root / "japan_expanded_v2_map" / "descriptor.mod",
            repo_root / "japan_expanded_v2_map.mod",
        )
        for path in descriptors:
            with self.subTest(path=path):
                document = parse_text(path.read_text(encoding="utf-8-sig"))
                supported = first_scalar(document.root, "supported_version")
...
class GeneralValidatorEncodingTests(unittest.TestCase):
    def test_bom_free_utf8_chinese_country_name_pools_are_counted_exactly(self) -> None:
        validator = (
            LIVE_MOD_ROOT.parent
            / "skills"
            / "eu4-modding"
            / "scripts"
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            mod_root = root / "utf8_country_pool"
            country_root = mod_root / "common" / "countries"
            game_root = root / "game"
            country_root.mkdir(parents=True)
            game_root.mkdir()
            (mod_root / "descriptor.mod").write_text(
                'name="UTF-8 country pool fixture"\n',
                encoding="utf-8",
            )
            (root / "utf8_country_pool.mod").write_text(
                'name="UTF-8 country pool fixture"\n'
                'path="mod/utf8_country_pool"\n',
                encoding="utf-8",
            )
            (country_root / "UTF8 Test.txt").write_text(
                "leader_names = {\n"
                "\t藤原 源 平 橘 北畠 三条 近卫 西园寺 德川 九条\n"
                "}\n"
                "ship_names = {\n"
                "\tYamato Ise Kii Kyoto Nara Heian Kinri Mikado Hachiman Kashihara\n"
                "}\n",
                encoding="utf-8",
            )
        ...
        self.assertNotIn("less than 10", result.stdout + result.stderr)

    def test_skill_powershell_path_text_reads_declare_encoding(self) -> None:
        scripts_root = LIVE_MOD_ROOT.parent / "skills" / "eu4-modding" / "scripts"
        violations: list[str] = []
        for path in sorted(scripts_root.glob("*.ps1")):
            for line_number, line in enumerate(
                path.read_text(encoding="utf-8-sig").splitlines(), start=1
            ):
                if ("Get-Content" in line or "Select-String" in line) and (
                    "-LiteralPath" in line or "-Path" in line
                ):
...
class SharedLedgerTests(unittest.TestCase):
    def _copy_coordination_surface(self, destination: Path) -> Path:
        ...
        (main_root / "tools" / "jxp_runtime_acceptance").mkdir(parents=True)
    ...
    def test_shared_ledger_rejects_unindexed_report(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
    ...
    def test_shared_ledger_rejects_schema_drift(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
    ...
    def test_shared_ledger_requires_every_stable_todo(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8").replace(
                "| JXP-023 | P0 | IN_PROGRESS |",
                "| JXP-099 | P0 | IN_PROGRESS |",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
    ...
    def test_shared_ledger_rejects_r5_identity_drift(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
    ...
    def test_shared_ledger_rejects_runtime_matrix_hash_drift(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            matrix = (
                root / "tools" / "jxp_runtime_acceptance" / "runtime_scenarios.json"
            )
    ...
    def test_authorized_runtime_status_requires_dated_launch_authorization(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8")
            self.assertIn("- Runtime acceptance: `PENDING_USER_APPROVAL`", text)
            text = text.replace(
                "- Runtime acceptance: `PENDING_USER_APPROVAL`",
                "- Runtime acceptance: `AUTHORIZED_NOT_STARTED`",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
    ...
    def test_shared_ledger_rejects_runtime_closure_phase_drift(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8").replace(
                "| JXP-008 | WAK 进入、投资、压力、退出、AI 与地图语义合同通过 | `R7` |",
                "| JXP-008 | WAK 进入、投资、压力、退出、AI 与地图语义合同通过 | `R6` |",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
    ...
    def test_shared_ledger_requires_every_active_runtime_closure_row(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            lines = ledger.read_text(encoding="utf-8").splitlines()
            lines = [line for line in lines if not line.startswith("| JXP-009 | IJP ")]
            ledger.write_text("\n".join(lines) + "\n", encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
        self.assertIn(
    ...
    def test_shared_ledger_rejects_external_registry_drift(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8").replace(
                "| `EXT-R5-PROV-001` | provenance JSON | original-machine recovery | "
                "`r5_external_input_provenance.json` | 2,326 |",
                "| `EXT-R5-PROV-001` | provenance JSON | original-machine recovery | "
                "`r5_external_input_provenance.json` | 2,327 |",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
    ...
    def test_shared_ledger_rejects_out_of_order_journal(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8").replace(
                "### 2026-07-13 - LEDGER-003 -",
                "### 2026-07-01 - LEDGER-003 -",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
    ...
    def test_shared_ledger_rejects_unowned_in_progress_todo(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8").replace(
                "| JXP-001 | P0 | IN_PROGRESS | CODEX_A |",
                "| JXP-001 | P0 | IN_PROGRESS | UNCLAIMED |",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
    ...
    def test_shared_ledger_rejects_done_todo_with_pending_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = self._copy_coordination_surface(Path(directory))
            self._write_descriptors(root, "0.28.1")
            ledger = root / "dev_logs" / "JXP_SHARED_DEVELOPMENT_LEDGER.md"
            text = ledger.read_text(encoding="utf-8").replace(
                "| JXP-003 | P0 | DONE | CODEX_ROOT | Balance/Compatibility | "
                "逐项重构 19 个 `num_of_cities = 25/30` 地图尺度敏感门槛 | "
                "每项记录历史意图，改为统一度、战略省份、area、发展或合理城市数；联合门禁 0 warning |",
                "| JXP-003 | P0 | DONE | CODEX_ROOT | Balance/Compatibility | "
                "逐项重构 19 个 `num_of_cities = 25/30` 地图尺度敏感门槛 | PENDING |",
                1,
            )
            ledger.write_text(text, encoding="utf-8")
            result = check_shared_development_ledger(ValidationContext(root))
...
class GovernmentReformRewriteTests(unittest.TestCase):
    def test_semantic_level_matrix_covers_active_reforms(self) -> None:
        ...
        daimyo_stage_ids = {
            reform_id
            for reform_id in all_ids
            if reform_id.startswith("jxp_reform_daimyo_")
        }
        final_state_ids = all_ids & set(FINAL_STATE_POLITICAL_REQUIREMENTS)
        japanese_track_ids = {
            reform_id
            for reform_id in all_ids
            if reform_id.startswith("jxp_reform_japanese_")
        }
        route_ids = (
            all_ids
            - founder_ids
        ...
        self.assertEqual(len(founder_ids), EXPECTED_FOUNDER_COUNT)
        self.assertEqual(len(daimyo_stage_ids), EXPECTED_DAIMYO_STAGE_COUNT)
        self.assertEqual(len(japanese_track_ids), EXPECTED_JAPANESE_TRACK_COUNT)
        self.assertEqual(
            final_state_ids,
            set(FINAL_STATE_POLITICAL_REQUIREMENTS),
        )
        self.assertEqual(
            len(final_state_ids),
            EXPECTED_FINAL_STATE_POLITICAL_COUNT,
        )
        self.assertEqual(len(route_ids), EXPECTED_VISIBLE_ROUTE_COUNT)
        self.assertTrue(founder_ids.isdisjoint(daimyo_stage_ids))
        self.assertTrue(founder_ids.isdisjoint(final_state_ids))
        self.assertTrue(founder_ids.isdisjoint(route_ids))
        self.assertTrue(daimyo_stage_ids.isdisjoint(final_state_ids))
        self.assertTrue(daimyo_stage_ids.isdisjoint(route_ids))
        self.assertTrue(final_state_ids.isdisjoint(route_ids))
        self.assertTrue(
            all(
                not reform_id.startswith("jxp_reform_founder_")
                and not reform_id.startswith("jxp_reform_final_")
                for reform_id in route_ids
            )
        )
        self.assertIn(
...
"""
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("jxp_final_state_open_trigger = yes", first)
            self.assertNotIn("jxp_is_japanese_polity_trigger = yes", first)

    def test_route_reform_matrix_rejects_broad_cross_route_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for relative in ("common", "events", "decisions"):
            ...
            route_file = (
                ...
                / "common"
                / "government_reforms"
                / "jxp_18_route_reforms_extra.txt"
            )
            text = route_file.read_text(encoding="utf-8-sig")
            ...
            route_file.write_text(changed, encoding="utf-8")
            codes = {
                issue.code
                for issue in check_reforms(ValidationContext(root)).issues
            }
        self.assertIn("reform.route_broad_potential", codes)
        self.assertIn("reform.profile_visibility", codes)
...
"""
    ...
    def test_runtime_fingerprint_is_parseable_and_keeps_legacy_anchors(self) -> None:
        ...
        self.assertIn("has_mission = control_spice_trade_asia", rendered)
