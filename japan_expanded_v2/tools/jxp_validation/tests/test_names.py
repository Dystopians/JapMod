...
class JapaneseNameAuditTests(unittest.TestCase):
    def test_live_main_and_companion_name_surface_is_clean(self) -> None:
        ...
        self.assertEqual(19, audit.stats["gameplay_source_pairs"])
        self.assertEqual(87, audit.stats["gameplay_encoded_files"])
    ...
    def test_raw_utf8_cjk_gameplay_name_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            main = repo / "japan_expanded_v2"
            companion = repo / "japan_expanded_v2_map"
            ...
            companion.mkdir()
            (history / "CJP - Confucian Wa.txt").write_text(
                '1444.11.11 = { monarch = { name = "家康" dynasty = "德川" } }\n',
                encoding="utf-8",
            )
            audit = audit_japanese_names(main, companion, LIVE_REPO_ROOT)
        self.assertIn(
