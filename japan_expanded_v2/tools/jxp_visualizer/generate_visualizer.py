...
FINAL_STATE_REFORM_SOURCE = Path("common/government_reforms/jxp_80_final_state_political_reforms.txt")
FINAL_STATE_REFORM_PREFIX = "jxp_reform_final_"
FINAL_STATE_REFORM_TIER = "absolute_rule_vs_constitutional"
FINAL_STATE_POWER_TIER = "feudalism_vs_autocracy"
FINAL_STATE_POWER_TIER_LABEL = "国家权力结构"
CELESTIAL_REFORM_ID = "celestial_empire"
FINAL_STATE_POWER_SOURCES = (
    Path("common/government_reforms/jxp_79_final_state_power_structures.txt"),
    Path("common/government_reforms/jxp_japanese_reforms.txt"),
)
FINAL_STATE_EOC_MODIFIER_SOURCE = Path(
    "common/triggered_modifiers/jxp_82_eoc_final_state_power_structures.txt"
)
FINAL_STATE_POWER_CONTRACTS = {
...
FINAL_STATE_REFORM_TIER_LABEL = "政治改革：专制与宪政"
FINAL_STATE_TRIGGER_LABELS = {
    "jxp_final_state_uncommitted_trigger": "尚未选择终局道路的日本",
    "jxp_final_state_sakoku_trigger": "已选择锁国道路的日本",
    "jxp_final_state_open_trigger": "已选择开国道路的日本",
    "jxp_final_state_kirishitan_trigger": "吉利支丹日本（KJP）",
    "jxp_final_state_confucian_trigger": "朱子礼制日本（CJP）",
    "jxp_final_state_imperial_trigger": "王政复古日本（EJP）",
    "jxp_final_state_reformed_trigger": "御言盟约日本（RFJ）",
    "jxp_final_state_kaikyo_trigger": "海教日本（SJP）",
    "jxp_final_state_ikko_trigger": "一向宗共和国（IJP）",
    "jxp_final_state_wokou_trigger": "倭寇联邦（WAK）",
}
FINAL_STATE_TRIGGER_LABELS.update(
    {
        trigger_id.replace("_trigger", "_power_active_trigger"): f"{label}：规范国体已生效"
        for trigger_id, label in tuple(FINAL_STATE_TRIGGER_LABELS.items())
    }
)
COUNTRY_FLAG_LABELS = {
    "jxp_path_sakoku": "锁国道路",
    "jxp_path_open_trade": "开国通商道路",
    "jxp_path_kirishitan": "吉利支丹道路",
    "jxp_path_confucian": "朱子礼制道路",
    "jxp_path_imperial": "王政复古道路",
    "jxp_path_reformed": "御言盟约道路",
    "jxp_path_kaikyo": "海教道路",
    "jxp_path_ikko": "一向宗道路",
    "jxp_path_wokou": "倭寇联邦道路",
}
GOVERNMENT_REFORM_FALLBACKS = {
    "celestial_empire": "天朝政体",
    "military_dictatorship_reform": "军事独裁制",
    "presidential_despot_reform": "总统独裁制",
    "quash_noble_power_reform": "限制贵族特权",
}
...
def scripted_trigger_name(ctx: VisualContext, key: str) -> str:
    return FINAL_STATE_TRIGGER_LABELS.get(key, loc_name(ctx, key, humanize_id(key)))


def country_flag_name(value: str) -> str:
    return COUNTRY_FLAG_LABELS.get(value, humanize_id(value))


def government_reform_name(ctx: VisualContext, key: str) -> str:
    fallback = GOVERNMENT_REFORM_FALLBACKS.get(key, humanize_id(key))
    return loc_name(ctx, key, fallback)
...
MODIFIER_LABELS: dict[str, tuple[str, str]] = {
    ...
    "all_estate_loyalty_equilibrium": ("各阶层忠诚均衡", "percent"),
    "artillery_power": ("炮兵作战能力", "percent"),
    "build_cost": ("建造花费", "percent"),
    "burghers_loyalty_modifier": ("市民阶层忠诚均衡", "percent"),
    "capture_ship_chance": ("俘获敌舰几率", "percent"),
    "caravan_power": ("商队力量", "percent"),
    "church_loyalty_modifier": ("教士阶层忠诚均衡", "percent"),
    ...
    "global_ship_recruit_speed": ("造船时间", "percent"),
    ...
    "yearly_absolutism": ("年度专制度", "flat1"),
    "yearly_corruption": ("年度腐败", "flat2"),
...
def format_modifier_line(key: str, value: str | None) -> dict:
    ...
    decimals = {"flat1": 1, "flat2": 2}.get(kind, 0)
    ...
    if key in {"global_ship_recruit_speed", "yearly_corruption"}:
        tone = "good" if rendered.startswith("-") else "bad"
...
def summarize_item(item: Item, ctx: VisualContext, mode: str, depth: int,
    ...
    if mode == "trigger" and key in ctx.scripted_triggers and text == "yes":
        label = scripted_trigger_name(ctx, key)
...
def summarize_effect_item(key: str, value: object, text: str | None, ctx: VisualContext,
    ...
    if key in {"add_government_reform", "remove_government_reform"} and text:
        verb = "采用政府改革" if key == "add_government_reform" else "移除政府改革"
        reform_name = government_reform_name(ctx, text)
        tone = "good" if key == "add_government_reform" else "bad"
        return [make_tip(f"{verb}：{reform_name}", tone)]
    ...
    if key in {"set_country_flag", "clr_country_flag"} and text:
        ...
        return [make_tip(f"{verb}：{country_flag_name(text)}", "muted")]
...
def summarize_trigger_item(key: str, value: object, text: str | None, ctx: VisualContext,
    ...
    if key == "has_country_flag" and text:
        return [make_tip(f"已进入剧情状态：{country_flag_name(text)}", "muted")]
    ...
    if key == "has_reform" and text:
        return [make_tip(f"拥有政府改革：{government_reform_name(ctx, text)}", "need")]
    if key == "has_dlc" and text:
        return [make_tip(f"启用 DLC：{text}", "need")]
    if key == "is_emperor_of_china" and text in {"yes", "no"}:
        return [make_tip("是中华天子" if text == "yes" else "不是中华天子", "need")]
...
def load_reform_tiers(mod_root: Path, warnings: list[str]) -> dict[str, list[str]]:
    path = mod_root / "common" / "governments" / "00_governments.txt"
    if not path.exists():
        warnings.append(f"Missing government tier registry: {rel(path, mod_root)}")
        return {}
    try:
        root = parse_clausewitz(path)
    except ClausewitzParseError as exc:
    ...
    def visit(obj: CwObject) -> None:
        for item in obj.entries:
            if not item.key or not isinstance(item.value, CwObject):
                continue
            reforms = first(item.value, "reforms")
            for reform_id in scalar_list(reforms):
                tiers.setdefault(reform_id, set()).add(item.key)
            visit(item.value)

    visit(root)
    return {reform_id: sorted(names) for reform_id, names in tiers.items()}


def find_scripted_trigger_calls(value: object | None, ctx: VisualContext) -> list[str]:
    calls: list[str] = []
    if not isinstance(value, CwObject):
        return calls
    for item in value.entries:
        if item.key in ctx.scripted_triggers and as_text(item.value) == "yes":
            calls.append(item.key)
        calls.extend(find_scripted_trigger_calls(item.value, ctx))
    return calls


def png_data_uri(path: Path, reform_id: str) -> str:
    if not path.is_file():
        raise FileNotFoundError(f"Missing reform preview icon for {reform_id}: {path}")
    payload = path.read_bytes()
    if not payload.startswith(b"\x89PNG\r\n\x1a\n"):
        raise ValueError(f"Reform preview icon is not a PNG for {reform_id}: {path}")
    return "data:image/png;base64," + base64.b64encode(payload).decode("ascii")


def extract_modifier_entries(obj: CwObject | None) -> list[dict]:
    entries: list[dict] = []
    if not isinstance(obj, CwObject):
        return entries
    for modifier in obj.entries:
        value = as_text(modifier.value)
        if not modifier.key or value is None:
            continue
        entries.append(
            {
                "id": modifier.key,
                "value": value,
                "tip": format_modifier_line(modifier.key, value),
            }
        )
    return entries


def load_top_level_objects(
    mod_root: Path,
    sources: Iterable[Path],
    warnings: list[str],
) -> dict[str, tuple[CwObject, Path, int]]:
    objects: dict[str, tuple[CwObject, Path, int]] = {}
    for relative in sources:
        path = mod_root / relative
        if not path.is_file():
            warnings.append(f"Missing final-state review source: {rel(path, mod_root)}")
            continue
        try:
            root = parse_clausewitz(path)
        except ClausewitzParseError as exc:
        ...
        for item in root.entries:
            ...
            if item.key in objects:
                warnings.append(f"Duplicate final-state review object: {item.key}")
                continue
            objects[item.key] = (item.value, relative, item.line)
    return objects


def extract_final_state_reforms(
    mod_root: Path,
    ctx: VisualContext,
    missing_loc: set[str],
    warnings: list[str],
) -> list[dict]:
    path = mod_root / FINAL_STATE_REFORM_SOURCE
    if not path.exists():
        warnings.append(f"Missing final-state reform source: {rel(path, mod_root)}")
        return []
    try:
        root = parse_clausewitz(path)
    except ClausewitzParseError as exc:
    ...
    tier_registry = load_reform_tiers(mod_root, warnings)
    power_objects = load_top_level_objects(
        mod_root, FINAL_STATE_POWER_SOURCES, warnings
    )
    eoc_modifier_objects = load_top_level_objects(
        mod_root, (FINAL_STATE_EOC_MODIFIER_SOURCE,), warnings
    )
    reforms: list[dict] = []
    for item in root.entries:
        if not item.key or not item.key.startswith(FINAL_STATE_REFORM_PREFIX) or not isinstance(item.value, CwObject):
            continue
        obj = item.value
        title_key = item.key
        desc_key = f"{item.key}_desc"
        ...
        potential = first(obj, "potential")
        trigger = first(obj, "trigger")
        icon = as_text(first(obj, "icon")) or ""
        icon_file = Path("gfx/interface/government_reform_icons/source/previews/icons") / f"{icon}.png"
        icon_data_uri = png_data_uri(mod_root / icon_file, item.key)

        tier_candidates = tier_registry.get(item.key, [])
        tier = FINAL_STATE_REFORM_TIER if FINAL_STATE_REFORM_TIER in tier_candidates else (tier_candidates[0] if tier_candidates else "")
        if tier != FINAL_STATE_REFORM_TIER:
            warnings.append(
                f"Final-state reform {item.key} is not registered in {FINAL_STATE_REFORM_TIER}; "
                f"found {', '.join(tier_candidates) or 'no tier'}"
            )

        modifiers = first(obj, "modifiers")
        modifier_entries = extract_modifier_entries(
            modifiers if isinstance(modifiers, CwObject) else None
        )
        if len(modifier_entries) != 2:
            warnings.append(f"Final-state reform {item.key} has {len(modifier_entries)} modifiers; expected exactly 2")

        trigger_calls = sorted(set(
            call for call in (*find_scripted_trigger_calls(potential, ctx), *find_scripted_trigger_calls(trigger, ctx))
            if call in FINAL_STATE_TRIGGER_LABELS
        ))
        state_trigger = trigger_calls[0] if len(trigger_calls) == 1 else ""
        power_review: dict = {}
        eoc_review: dict = {}
        contract = FINAL_STATE_POWER_CONTRACTS.get(state_trigger)
        if contract is None:
            warnings.append(
                f"Final-state political reform {item.key} has no power-equivalence contract"
            )
        else:
            power_id, eoc_modifier_id = contract
            power_record = power_objects.get(power_id)
            eoc_record = eoc_modifier_objects.get(eoc_modifier_id)
            if power_record is None:
                warnings.append(f"Missing final-state power structure: {power_id}")
            else:
                power_obj, power_source, power_line = power_record
                power_title_key = power_id
                power_desc_key = f"{power_id}_desc"
                for key in (power_title_key, power_desc_key):
                    if key not in ctx.loc:
                        missing_loc.add(key)
                power_icon = as_text(first(power_obj, "icon")) or ""
                power_icon_file = (
                    Path("gfx/interface/government_reform_icons/source/previews/icons")
                    / f"{power_icon}.png"
                )
                power_tiers = tier_registry.get(power_id, [])
                power_tier = (
                    FINAL_STATE_POWER_TIER
                    if FINAL_STATE_POWER_TIER in power_tiers
                    else (power_tiers[0] if power_tiers else "")
                )
                if power_tier != FINAL_STATE_POWER_TIER:
                    warnings.append(
                        f"Final-state power structure {power_id} is not registered in "
                        f"{FINAL_STATE_POWER_TIER}; found "
                        f"{', '.join(power_tiers) or 'no tier'}"
                    )
                power_modifiers = extract_modifier_entries(
                    first(power_obj, "modifiers")
                    if isinstance(first(power_obj, "modifiers"), CwObject)
                    else None
                )
                power_active_trigger_id = state_trigger.replace(
                    "_trigger", "_power_active_trigger"
                )
                power_active = ctx.scripted_triggers.get(power_active_trigger_id)
                power_potential = first(power_obj, "potential")
                power_review = {
                    "id": power_id,
                    "title": loc_value(ctx.loc, power_title_key, power_id),
                    "desc": loc_value(ctx.loc, power_desc_key, ""),
                    "icon": power_icon,
                    "iconDataUri": png_data_uri(
                        mod_root / power_icon_file, power_id
                    ),
                    "tier": power_tier,
                    "tierLabel": (
                        FINAL_STATE_POWER_TIER_LABEL
                        if power_tier == FINAL_STATE_POWER_TIER
                        else humanize_id(power_tier)
                    ),
                    "potentialTips": summarize_script(
                        power_potential, ctx, "trigger"
                    )
                    if power_potential
                    else [],
                    "powerActiveTrigger": power_active_trigger_id,
                    "powerActiveTips": summarize_script(
                        power_active, ctx, "trigger"
                    )
                    if power_active
                    else [],
                    "modifierEntries": power_modifiers,
                    "modifierTips": [entry["tip"] for entry in power_modifiers],
                    "file": power_source.as_posix(),
                    "line": power_line,
                }
                if len(power_modifiers) != 3:
                    warnings.append(
                        f"Final-state power structure {power_id} has "
                        f"{len(power_modifiers)} modifiers; expected exactly 3"
                    )
            if eoc_record is None:
                warnings.append(f"Missing final-state EoC modifier: {eoc_modifier_id}")
            else:
                eoc_obj, eoc_source, eoc_line = eoc_record
                if eoc_modifier_id not in ctx.loc:
                    missing_loc.add(eoc_modifier_id)
                eoc_potential = first(eoc_obj, "potential")
                eoc_trigger = first(eoc_obj, "trigger")
                eoc_modifiers = extract_modifier_entries(eoc_obj)
                eoc_review = {
                    "id": eoc_modifier_id,
                    "title": loc_value(ctx.loc, eoc_modifier_id, eoc_modifier_id),
                    "identity": CELESTIAL_REFORM_ID,
                    "identityTitle": government_reform_name(ctx, CELESTIAL_REFORM_ID),
                    "potentialTips": summarize_script(
                        eoc_potential, ctx, "trigger"
                    )
                    if eoc_potential
                    else [],
                    "triggerTips": summarize_script(
                        eoc_trigger, ctx, "trigger"
                    )
                    if eoc_trigger
                    else [],
                    "modifierEntries": eoc_modifiers,
                    "modifierTips": [entry["tip"] for entry in eoc_modifiers],
                    "mirrorMatches": False,
                    "mirrorMatchedCount": 0,
                    "mirrorExpectedCount": 3,
                    "file": eoc_source.as_posix(),
                    "line": eoc_line,
                }
                if len(eoc_modifiers) != 3:
                    warnings.append(
                        f"Final-state EoC modifier {eoc_modifier_id} has "
                        f"{len(eoc_modifiers)} modifiers; expected exactly 3"
                    )
            if power_review and eoc_review:
                power_signature = sorted(
                    (entry["id"], entry["value"])
                    for entry in power_review["modifierEntries"]
                )
                eoc_signature = sorted(
                    (entry["id"], entry["value"])
                    for entry in eoc_review["modifierEntries"]
                )
                matched_count = len(set(power_signature) & set(eoc_signature))
                mirror_matches = power_signature == eoc_signature and len(power_signature) == 3
                eoc_review["mirrorMatches"] = mirror_matches
                eoc_review["mirrorMatchedCount"] = matched_count
                if not mirror_matches:
                    warnings.append(
                        f"Final-state EoC modifier {eoc_modifier_id} does not mirror "
                        f"{power_id}: {eoc_signature} != {power_signature}"
                    )
        reforms.append({
            "id": item.key,
            "titleKey": title_key,
            ...
            "desc": loc_value(ctx.loc, desc_key, ""),
            "icon": icon,
            "iconFile": icon_file.as_posix(),
            "iconDataUri": icon_data_uri,
            "tier": tier,
            "tierLabel": FINAL_STATE_REFORM_TIER_LABEL if tier == FINAL_STATE_REFORM_TIER else humanize_id(tier),
            "tierCandidates": tier_candidates,
            "stateTrigger": state_trigger,
            "stateLabel": scripted_trigger_name(ctx, state_trigger) if state_trigger else "",
            "potential": render_value(potential, 0) if potential else "",
            "potentialTips": summarize_script(potential, ctx, "trigger") if potential else [],
            "trigger": render_value(trigger, 0) if trigger else "",
            "triggerTips": summarize_script(trigger, ctx, "trigger") if trigger else [],
            "modifierEntries": modifier_entries,
            "modifierTips": [entry["tip"] for entry in modifier_entries],
            "powerStructure": power_review,
            "eocPowerModifier": eoc_review,
            "file": rel(path, mod_root),
            "line": item.line,
        })

    if len(reforms) != 10:
        warnings.append(f"Final-state political reform collection has {len(reforms)} entries; expected exactly 10")
    return reforms
...
def collect_files(mod_root: Path) -> dict[str, list[str]]:
    ...
    reform_paths = (
        FINAL_STATE_REFORM_SOURCE,
        *FINAL_STATE_POWER_SOURCES,
    )
    result["government_reforms"] = [
        rel(mod_root / path, mod_root)
        for path in reform_paths
        if (mod_root / path).exists()
    ]
    result["triggered_modifiers"] = [
        rel(mod_root / FINAL_STATE_EOC_MODIFIER_SOURCE, mod_root)
    ] if (mod_root / FINAL_STATE_EOC_MODIFIER_SOURCE).exists() else []
    return result
...
def build_data(
    ...
    *,
    include_final_state_reforms: bool | None = None,
) -> dict:
    ...
    if include_final_state_reforms is None:
        include_final_state_reforms = (
            mod_root.resolve() == MOD_ROOT.resolve()
            or (mod_root / FINAL_STATE_REFORM_SOURCE).is_file()
        )
    final_state_reforms = (
        extract_final_state_reforms(mod_root, ctx, missing_loc, warnings)
        if include_final_state_reforms
        else []
    )
    data = {
        ...
        "finalStateReforms": final_state_reforms,
        "decisions": decisions,
        "events": events,
        "warnings": warnings,
    }
    data["counts"] = {
        ...
        "finalStateReforms": len(final_state_reforms),
        "finalStatePowerStructures": sum(
            bool(reform.get("powerStructure")) for reform in final_state_reforms
        ),
        "finalStateEocModifiers": sum(
            bool(reform.get("eocPowerModifier")) for reform in final_state_reforms
        ),
...
.reform-hero-icon {{
  ...
  height: 72px;
  object-fit: cover;
  border: 3px solid #8d6938;
  box-shadow: 0 0 0 1px #3f2818, 0 7px 14px rgba(50, 27, 14, .28);
  background: #17110d;
}}
.reform-hero h2 {{
  margin-top: 0;
...
<div class="shell">
  <header class="topbar">
    ...
    <div class="brand">
      <h1>{title}</h1>
      <div class="sub">任务、政府改革、决议、事件可视化审阅器 · 生成时间 {generated}</div>
    </div>
...
let state = {{
  ...
  mission: "",
  reform: DATA.finalStateReforms[0]?.id || "",
  decision: "",
...
function textMatches(item, query) {{
  ...
  const hay = [
    item.id, item.title, item.desc, item.file, item.category, item.tier, item.tierLabel, item.stateLabel,
    item.titleKey, item.descKey, item.picture, item.icon,
    ...(item.modifierEntries || []).flatMap(modifier => [modifier.id, modifier.value, modifier.tip?.text]),
    item.powerStructure?.id, item.powerStructure?.title, item.powerStructure?.desc,
    ...(item.powerStructure?.modifierEntries || []).flatMap(modifier => [modifier.id, modifier.value, modifier.tip?.text]),
    item.eocPowerModifier?.id, item.eocPowerModifier?.title, item.eocPowerModifier?.identity,
    ...(item.eocPowerModifier?.modifierEntries || []).flatMap(modifier => [modifier.id, modifier.value, modifier.tip?.text]),
  ...
  return `
    <section class="panel detail">
      ...
      <div class="stats">
        ...
        <div class="stat"><strong>${{c.finalStateReforms}}</strong><span>终局政治改革</span></div>
        <div class="stat"><strong>${{c.finalStatePowerStructures}}</strong><span>路线权力结构</span></div>
        <div class="stat"><strong>${{c.finalStateEocModifiers}}</strong><span>天朝镜像层</span></div>
...
function renderReforms() {{
  const filtered = DATA.finalStateReforms.filter(item => textMatches(item, state.query));
  const selected = filtered.find(reform => reform.id === state.reform) || filtered[0];
  if (selected) state.reform = selected.id;
  return `<div class="toolbar">${{renderSearch()}}<div class="badge-row">${{badge(filtered.length + " / " + DATA.finalStateReforms.length + " 终局政治改革")}}</div></div>
    <div class="split">
      <aside class="panel side">
        ${{filtered.map(reform => `<button class="list-button ${{reform.id === state.reform ? "active" : ""}}" onclick="state.reform='${{reform.id}}'; render()">
          <span class="reform-list-heading">
            <img class="reform-list-icon" src="${{esc(reform.iconDataUri)}}" alt="">
            <span><strong>${{esc(reform.title)}}</strong><span class="small">${{esc(reform.stateLabel)}} · ${{esc(reform.tierLabel)}}</span></span>
          </span>
          ${{miniTips("采用后", reform.modifierTips, 2)}}
        </button>`).join("") || `<div class="empty">没有匹配的政府改革。</div>`}}
      </aside>
      <section class="panel detail">
        ${{selected ? renderReformDetail(selected) : `<div class="empty">没有读取到终局政治改革。</div>`}}
      </section>
    </div>`;
}}

function renderReformDetail(reform) {{
  return `<div class="reform-hero">
      <img class="reform-hero-icon" src="${{esc(reform.iconDataUri)}}" alt="${{esc(reform.title)}}">
      <div>
        <h2>${{esc(reform.title)}}</h2>
        <div class="badge-row">
          ${{badge(reform.id)}}
          ${{badge(reform.tierLabel)}}
          ${{badge("层级 " + reform.tier)}}
          ${{badge(fileLine(reform))}}
        </div>
      </div>
    </div>
    <p>${{esc(reform.desc || "无描述文本。")}}</p>
    ${{gamePreview("政府改革预览", [
      {{ title: "界面候选条件（potential）", tips: reform.potentialTips, script: "" }},
      {{ title: "可选条件（trigger）", tips: reform.triggerTips, script: "" }},
      {{ title: "采用后加成", tips: reform.modifierTips, script: "" }}
    ])}}
    ${{renderPowerStructureDetail(reform)}}`;
}}

function renderPowerStructureDetail(reform) {{
  const power = reform.powerStructure;
  const eoc = reform.eocPowerModifier;
  if (!power || !eoc) return `<div class="empty">缺少终局权力结构双态数据。</div>`;
  return `<div class="section-title">权力结构双态合同</div>
    <div class="reform-hero">
      <img class="reform-hero-icon" src="${{esc(power.iconDataUri)}}" alt="${{esc(power.title)}}">
      <div>
        <h2>${{esc(power.title)}}</h2>
        <div class="badge-row">
          ${{badge(power.id)}}
          ${{badge(power.tierLabel)}}
          ${{badge(fileLine(power))}}
        </div>
      </div>
    </div>
    <p>${{esc(power.desc || "无描述文本。")}}</p>
    ${{gamePreview("非天朝：路线一级权力结构", [
      {{ title: "生效条件", tips: power.potentialTips, script: "" }},
      {{ title: "终局任务判定", tips: power.powerActiveTips, script: "" }},
      {{ title: "路线国体加成", tips: power.modifierTips, script: "" }}
    ])}}
    ${{gamePreview("天朝：保留 celestial_empire 的等价层", [
      {{ title: "等价层条件", tips: eoc.potentialTips, script: "" }},
      {{ title: "月度生效判定", tips: eoc.triggerTips, script: "" }},
      {{ title: "镜像国体加成", tips: eoc.modifierTips, script: "" }}
    ])}}
    <div class="badge-row">
      ${{badge(eoc.title)}}
      ${{badge(eoc.id)}}
      ${{badge("天朝身份：" + eoc.identityTitle)}}
      ${{badge(eoc.mirrorMatches
        ? "数值镜像已验证 " + eoc.mirrorMatchedCount + " / " + eoc.mirrorExpectedCount
        : "数值不一致 " + eoc.mirrorMatchedCount + " / " + eoc.mirrorExpectedCount)}}
      ${{badge(fileLine(eoc))}}
    </div>`;
}}
...
function render() {{
  ...
  if (state.tab === "reforms") app.innerHTML = renderReforms();
...
def main() -> int:
    ...
    print(
        ...
        f"{data['counts']['finalStateReforms']} final-state reforms, "
