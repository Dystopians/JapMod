...
def _idea_registry_log(records: list[tuple[int, str]]) -> str:
    lines = [
        f"[ideas.cpp:491]: Idea Group #{group_id} tag = {tag} name = Fixture (7) ===>A B C D E F G"
        for group_id, tag in records
    ]
    lines.append("[eu4application.cpp:2032]: IdeaDatabase Initialized")
    return "\n".join(lines) + "\n"


def _visible_idea_registry_records() -> list[tuple[int, str]]:
    anchors = runtime.EXPECTED_VISIBLE_IDEA_REGISTRY_ANCHORS
    return [
        (group_id, anchors.get(group_id, f"fixture_{group_id:03d}_ideas"))
        for group_id in sorted(runtime.EXPECTED_VISIBLE_IDEA_REGISTRY_IDS)
    ]
...
def _write_r5_text_eu4(path: Path) -> tuple[Path, dict[str, object]]:
    """Write a compact full-text save with the historical R5 identity shape."""
    ...
    slots = [
        [
            "DOM_japanse_missions_1",
            "\ufeffjxp_japan_eastasia_missions",
            "\ufeffjxp_japan_state_missions",
        ],
        ["DOM_japanse_missions_2_a", "jxp_japan_pacific_missions"],
        ["DOM_japanse_missions_3_a", "jxp_japan_open_missions"],
        ["DOM_japanse_missions_4_a", "jxp_muslim_branch_missions"],
        ["DOM_japanse_missions_5", "jxp_frontier_diplomacy_missions"],
    ]
    legacy_series = (
        "jxp_daimyo_domain_missions",
        "jxp_ikko_route_missions",
        "jxp_japan_eastasia_missions",
        "jxp_japan_state_missions",
    ...
    lines = [
        ...
        "\tJAP={",
        "\t\tflags={",
        "\t\t\tjxp_path_open_trade=1444.12.18",
        "\t\t}",
        "\t\tcompleted_missions={",
        *[f'\t\t\t"{mission}"' for mission in completed],
        "\t\t}",
        "\t\tcountry_missions={",
    ]
    for slot in slots:
        lines.extend(("mission_slot={", " ".join(slot), "}"))
    lines.extend(("\t\t}", "\t}", "}", "legacy_series={"))
    lines.extend(f"\t\ufeff{name}" for name in legacy_series)
    lines.append("}")
    lines.append("filler={")
    for index in range(700):
        lines.append(
            f'\titem_{index}="{sha256(f"r5:{index}".encode()).hexdigest()}"'
        )
    lines.extend(("}", ""))
    ...
    identity: dict[str, object] = {
        ...
        "country_flags": {"jxp_path_open_trade": "1444.12.18"},
        "completed_jxp_missions": completed,
        "player_mission_slots": slots,
        "bom_series_counts": {
            name: payload.count(b"\xef\xbb\xbf" + name.encode("ascii"))
            for name in legacy_series
        },
    }
...
def _component(root: Path, key: str = "main") -> runtime.Component:
    ...
    version = "0.1.4-alpha" if key == "map" else "0.28.1"
    _write(
...
def _runtime_environment(root: Path):
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.enter_context(
            patch.object(
                runtime,
                "_require_isolated_user_data",
                side_effect=lambda value: runtime._require_ordinary_directory(
                    Path(value), "hermetic test acceptance root"
                ),
            )
        )
        ...
        stack.enter_context(
            patch.object(
                runtime, "_require_clean_control_plane", return_value="d" * 40
            )
        )
        stack.enter_context(
            patch.object(
...
class RuntimeAcceptanceTests(unittest.TestCase):
    def test_r3_pins_one_visible_final_state_reform_per_state(self) -> None:
        scenario = runtime._scenario("R3")
        manifest = scenario["expected_state_manifest"]
        expected = {
            "uncommitted": "jxp_reform_final_uncommitted_consensus",
            "sakoku": "jxp_reform_final_sakoku_constitution",
            "open": "jxp_reform_final_open_cabinet",
            "kirishitan": "jxp_reform_final_kirishitan_estates",
            "confucian": "jxp_reform_final_confucian_censorate",
            "imperial": "jxp_reform_final_imperial_daijokan",
            "reformed": "jxp_reform_final_reformed_synod",
            "kaikyo": "jxp_reform_final_kaikyo_diwan",
            "ikko": "jxp_reform_final_ikko_somon",
            "wokou": "jxp_reform_final_wokou_admiralty",
        }
        self.assertEqual(set(manifest), set(expected))
        self.assertEqual(
            {
                state: record["visible_political_reform"]
                for state, record in manifest.items()
            },
            expected,
        )
        self.assertEqual(
            {
                record["political_reform_tier"]
        ...
        self.assertTrue(
            all(
                len(record["political_reform_modifiers"]) == 2
                for record in manifest.values()
            )
        )
        contract = scenario["session_contract"]
        artifacts = {item["role"]: item for item in contract["required_artifacts"]}
        adopted = artifacts["r3_final_reform_adopted_screenshot_set"]
        reloaded = artifacts["r3_final_reform_reload_screenshot_set"]
        self.assertEqual(10, adopted["min_items"])
        self.assertEqual(
            {f"r3_adopt_{state}" for state in expected},
            set(adopted["case_ids"]),
        )
        self.assertEqual(4, reloaded["min_items"])
        self.assertEqual(
            {
                "r3_reload_sakoku",
                "r3_reload_confucian",
            ...
            set(reloaded["case_ids"]),
        )
        self.assertTrue(
            {
                "r3_final_reform_visibility",
                "r3_final_reform_adoption",
                "r3_final_reform_reload_stability",
            }.issubset(contract["assertion_ids"])
        )

    def test_r4_pins_real_pre_political_parent_lineage(self) -> None:
        scenario = runtime._scenario("R4")
        phases = scenario["session_phases"]
        pre = phases["legacy-028-pre-political"]
        current = phases["current-migrate"]
        self.assertEqual(
            {"main": "62902921c15a55a9306416db1880d1b09ad8de8f"},
            pre["required_source_revisions"],
        )
        self.assertEqual(10, pre["minimum_saves"])
        self.assertEqual(
            10,
            len(
                [
                    item
                    for item in pre["required_artifacts"]
                    if item["role"].endswith("_input_save")
                ]
            ),
        )
        current_parents = {
            item["role"]: (item["scenario"], item["phase"])
            for item in current["required_parent_collections"]
        }
        self.assertEqual(
            ("R4", "legacy-028-pre-political"),
            current_parents["r4_028_pre_political_collection"],
        )
        political_cases = [
            item
            for item in scenario["migration_cases"]
            if item["type"] == "final_state_political_reform_migration"
        ]
        self.assertEqual(10, len(political_cases))
        self.assertTrue(
            all(
                item["producer_phase"] == "legacy-028-pre-political"
                and item["consumer_phase"] == "current-migrate"
                and "r4_political_reform_migration_once" in item["assertion_ids"]
                for item in political_cases
            )
        )

    def test_r2_pins_fresh_monarchy_identity_and_authority_controls(self) -> None:
        scenario = runtime._scenario("R2")
        contract = scenario["session_contract"]
        self.assertEqual(23, contract["minimum_screenshots"])
        self.assertIn("r2_idea_registry_load", contract["assertion_ids"])
        manifest = scenario["expected_government_manifest"]
        self.assertEqual(
            {"RKK", "KYO", "HNM", "DHO", "TGS", "OTM", "MRI", "ASK", "RYU"},
            set(manifest),
        )
        for tag in ("RKK", "KYO", "HNM", "DHO", "TGS", "OTM", "MRI"):
            self.assertEqual("monarchy", manifest[tag]["government"])
            self.assertEqual("daimyo", manifest[tag]["tier_one_reform"])
            self.assertEqual("legitimacy", manifest[tag]["authority"])
            self.assertEqual(
                ["jxp_japanese_polity_mechanic"],
                manifest[tag]["required_mechanics"],
            )
        self.assertEqual("shogunate", manifest["ASK"]["tier_one_reform"])
        self.assertEqual(
            ["shogunate_mechanic", "jxp_japanese_polity_mechanic"],
            manifest["ASK"]["required_mechanics"],
        )
        self.assertEqual("autocracy_reform", manifest["RYU"]["tier_one_reform"])
        self.assertEqual(
            ["jxp_japanese_polity_mechanic"],
            manifest["RYU"]["forbidden_mechanics"],
        )
        artifacts = {item["role"]: item for item in contract["required_artifacts"]}
        for role in ("r2_ask_government_screenshot", "r2_ryu_government_screenshot"):
            self.assertIn("r2_profile_government", artifacts[role]["supports_assertion_ids"])
        self.assertIn(
            "countryreformview.cpp:289",
            next(
                item["text"]
                for item in scenario["acceptance"]
                if item["id"] == "r2_profile_government"
            ),
        )
        self.assertIn(
            "r2_idea_registry_load",
            artifacts["r2_setup_log"]["supports_assertion_ids"],
        )
        registry_acceptance = next(
            item["text"]
            for item in scenario["acceptance"]
            if item["id"] == "r2_idea_registry_load"
        )
        for anchor in (
            "#226 LIV_ideas",
            "#482 voc_ideas",
            ...
            self.assertIn(anchor, registry_acceptance)

    def test_r6_pins_all_eight_bridges_and_both_outcome_branches(self) -> None:
        scenario = runtime._scenario("R6")
        manifest = scenario["bridge_chain_manifest"]
        self.assertEqual(
            {
                "neo",
                "urban",
        ...
        for chain, record in manifest.items():
            self.assertEqual(
                expected_outcomes.get(chain, []),
                record["vanilla_outcome_flags"],
            )
            self.assertEqual(
                len(record["continuation_events"]), len(record["delay_days"])
            )
            self.assertTrue(all(days == 1095 for days in record["delay_days"]))
        cases = scenario["bridge_cases"]
        self.assertEqual(16, len(cases))
        by_chain = {
            chain: [item for item in cases if item["chain"] == chain]
            for chain in manifest
        }
        self.assertTrue(all(len(items) == 2 for items in by_chain.values()))
        for chain, outcomes in expected_outcomes.items():
            self.assertEqual(
                set(outcomes),
                {item["expected_outcome_flag"] for item in by_chain[chain]},
            )

    def test_r8_requires_bounded_natural_ai_entry_and_followup(self) -> None:
        scenario = runtime._scenario("R8")
        contract = scenario["session_contract"]
        self.assertIn("r8_ai_natural_behavior", contract["assertion_ids"])
        self.assertEqual(33, contract["minimum_screenshots"])
        artifacts = {item["role"]: item for item in contract["required_artifacts"]}
        for role in (
            "r8_ai_natural_entry_screenshot",
            "r8_ai_natural_followup_screenshot",
            "r8_observer_timeline",
        ):
            self.assertIn("r8_ai_natural_behavior", artifacts[role]["supports_assertion_ids"])
        steps = "\n".join(scenario["steps"])
        self.assertIn("at most thirty years", steps)
        self.assertIn("keep r8_ai_natural_behavior PENDING", steps)
        assertion = next(
            item
            for item in scenario["acceptance"]
            if item["id"] == "r8_ai_natural_behavior"
        )
        self.assertIn("naturally convenes", assertion["text"])
        self.assertIn("governance investment or legal exit", assertion["text"])

    def test_setup_log_requires_exact_engine_visible_idea_registry_shape(self) -> None:
        complete = _visible_idea_registry_records()
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            clean = _write(root / "setup.log", _idea_registry_log(complete))
            clean_codes = {item["code"] for item in runtime._scan_logs([clean])}
            self.assertFalse(
                {
                    "idea_registry_visible_shape",
                    "idea_registry_duplicate_id",
                    "idea_registry_duplicate_group",
                    "idea_registry_anchor_mismatch",
                }
                & clean_codes
            )
            ...
            truncated = _write(
                root / "setup.log",
                _idea_registry_log(complete[:14]),
            )
            truncated_codes = {
                item["code"] for item in runtime._scan_logs([truncated])
            }
            self.assertIn("idea_registry_visible_shape", truncated_codes)

            missing = _write(root / "setup.log", _idea_registry_log(complete[:-1]))
            missing_codes = {item["code"] for item in runtime._scan_logs([missing])}
            self.assertIn("idea_registry_visible_shape", missing_codes)

            duplicate = _write(
                root / "setup.log",
                _idea_registry_log(complete + [(226, "duplicate_ideas")]),
            )
            duplicate_codes = {
                item["code"] for item in runtime._scan_logs([duplicate])
            }
            self.assertIn("idea_registry_duplicate_id", duplicate_codes)

            duplicate_tag_records = list(complete)
            duplicate_tag_records[1] = (
                duplicate_tag_records[1][0],
                duplicate_tag_records[0][1],
            )
            duplicate_tag = _write(
                root / "setup.log",
                _idea_registry_log(duplicate_tag_records),
            )
            duplicate_tag_codes = {
                item["code"] for item in runtime._scan_logs([duplicate_tag])
            }
            self.assertIn("idea_registry_duplicate_group", duplicate_tag_codes)

            bad_anchor_records = [
                (group_id, "wrong_anchor_ideas" if group_id == 226 else tag)
                for group_id, tag in complete
            ]
            bad_anchor = _write(
                root / "setup.log",
                _idea_registry_log(bad_anchor_records),
            )
            bad_anchor_codes = {
                item["code"] for item in runtime._scan_logs([bad_anchor])
            }
            self.assertIn("idea_registry_anchor_mismatch", bad_anchor_codes)

    def test_setup_log_rejects_vanilla_otm_override_signature(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            broken = _write(
                root / "setup.log",
                "[ideas.cpp:491]: Idea Group #291 tag = OTM_ideas name = Otomo "
                "(7) ===>Unite the Clans Lord of Bungo Loyal Retainers "
                "Welcoming the Nanban Trade Christian Converts Tensho Embassy "
                "Kunikuzushi\n",
            )
            codes = {item["code"] for item in runtime._scan_logs([broken])}
            self.assertIn("vanilla_otm_idea_override", codes)

            clean = _write(
                root / "clean-setup.log",
                "[ideas.cpp:491]: Idea Group #291 tag = OTM_ideas name = JXP "
                "(7) ===>Bungo Registers Funai Guilds Nanban Compact "
                "Kyushu Cannon Foundry Sorin Embassy Port Magistrates House Law\n",
            )
            clean_codes = {item["code"] for item in runtime._scan_logs([clean])}
            self.assertNotIn("vanilla_otm_idea_override", clean_codes)

    def test_log_scan_rejects_dynamic_token_and_country_name_pool_errors(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            broken = _write(
                root / "error.log",
                "[ck2text.cpp:267]: Unknown text property 'Vaishya' for tag "
                "'[Country.GetVaishyaName]'\n"
                "[countrydatabase.cpp:272]: RKK has less than 10 random ship names\n",
            )
            codes = {item["code"] for item in runtime._scan_logs([broken])}
            self.assertIn("unknown_text_property", codes)
            self.assertIn("country_name_pool", codes)

            clean = _write(
                root / "clean.log",
                "vaisyas_loyalty_modifier uses [Country.GetVaishyasName]\n"
                "RKK has 32 random ship names\n",
            )
            clean_codes = {item["code"] for item in runtime._scan_logs([clean])}
            self.assertNotIn("unknown_text_property", clean_codes)
            self.assertNotIn("country_name_pool", clean_codes)

    def test_log_scan_rejects_government_reform_tier_overflow_but_not_box_probe(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            broken = _write(
                root / "error.log",
                "[countryreformview.cpp:289]: Trying to add more than 16 "
                "government reforms to one tier. Reforms over the limit will not be loaded\n",
            )
            codes = {item["code"] for item in runtime._scan_logs([broken])}
            self.assertIn("government_reform_tier_overflow", codes)

            sequential_probe = _write(
                root / "probe.log",
                "[gui.cpp:473]: countrygovernmentview.gui: Missing Overlapping "
                "Elements Box 'modifiers_box_5'\n",
            )
            probe_codes = {
                item["code"] for item in runtime._scan_logs([sequential_probe])
            }
            self.assertNotIn("government_reform_tier_overflow", probe_codes)

    def test_map_scan_ignores_opinion_key_but_keeps_real_map_failures(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            benign = _write(
                root / "setup.log",
                "[opinion_modifier.cpp:339]: Opinion Modifier #1005 "
                "tag = failed_province_purchase name = Failed Province Purchase\n",
            )
            benign_codes = {
                item["code"] for item in runtime._scan_logs([benign])
            }
            self.assertNotIn("map_initialization", benign_codes)

            harmless_core_removal = _write(
                root / "core-warning.log",
                "[province.cpp:9468]: Failed to remove prov 413 as core for "
                "AKK, it hasnt got it!\n",
            )
            harmless_codes = {
                item["code"]
                for item in runtime._scan_logs([harmless_core_removal])
            }
            self.assertNotIn("map_initialization", harmless_codes)
            self.assertNotIn("jxp_owned_province_core_remove", harmless_codes)

            owned_core_removal = _write(
                root / "jxp-core-warning.log",
                "[province.cpp:9468]: Failed to remove prov 4181 as core for "
                "OTM, it hasnt got it!\n",
            )
            owned_codes = {
                item["code"] for item in runtime._scan_logs([owned_core_removal])
            }
            self.assertIn("jxp_owned_province_core_remove", owned_codes)

            broken = _write(
                root / "broken-setup.log",
                "[map.cpp:417]: Error: map initialization failed for province "
                "definition 4982\n",
            )
            broken_codes = {
                item["code"] for item in runtime._scan_logs([broken])
            }
            self.assertIn("map_initialization", broken_codes)

    def test_default_acceptance_userdir_path_is_unambiguous(self) -> None:
        self.assertEqual(Path(r"C:\JXP_Acceptance"), runtime.DEFAULT_ACCEPTANCE_USER_DATA)
    ...
    def test_probe_manual_forbids_buffered_first_run_input(self) -> None:
        readme = (
            TOOLS_ROOT / "jxp_runtime_acceptance" / "README.md"
        ).read_text(encoding="utf-8")
        normalized = " ".join(readme.split())
        for required in (
            "does not inherit the daily Documents",
            "do not mean that the Mod VFS failed to load",
            "click the intended choice",
            "Do not press or buffer `Enter`, `Space`",
            "send no further input",
        ):
            self.assertIn(required, normalized)

    def test_candidate_pin_manual_requires_two_stage_commits(self) -> None:
        readme = (
            TOOLS_ROOT / "jxp_runtime_acceptance" / "README.md"
        ).read_text(encoding="utf-8")
        normalized = " ".join(readme.split())
        for required in (
            "cannot be created by one self-referential commit",
            "payload commit A",
            "control-plane commit B",
            "Without changing any runtime-whitelist byte",
            "git diff --quiet <A> -- <all runtime roots>",
            "Any runtime-byte change after commit A invalidates A",
            "A false `current_candidate.ready` is a hard stop",
            "omitting `--revision` would resolve to control-plane commit B",
        ):
            self.assertIn(required, normalized)
    ...
    def test_twelve_pin_records_include_three_protocol_and_nine_gameplay_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            with _runtime_environment(Path(temporary)) as env:
                ...
                self.assertTrue(preflight["current_candidate"]["ready"])
                with patch.object(
                    runtime,
                    "_current_candidate_readiness",
    ...
    def test_current_candidate_readiness_requires_clean_matching_ancestry(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary).resolve()
            repo = root / "repo"
            repo.mkdir()
            component = _component(repo / "components", "main")
            live_manifest = tuple(
                runtime._normalized_manifest_record(item)
                for item in runtime._manifest(component)
            )
            candidate = "a" * 40
            toolchain = "b" * 40
            ...
            def clean_git(_repo: Path, *args: str) -> str:
                if args == ("rev-parse", "HEAD"):
                    return toolchain
                if args == ("rev-parse", f"{candidate}^{{commit}}"):
                    return candidate
                if args == ("status", "--short", "--untracked-files=all"):
                    return ""
                if args == ("merge-base", candidate, toolchain):
                    return candidate
                if args[:4] == ("diff", "--name-only", "--no-renames", candidate):
                    return ""
                self.fail(f"unexpected Git request: {args!r}")
            ...
            common = (
                ...
                patch.object(
                    runtime, "_current_candidate_revision", return_value=candidate
                ),
                patch.object(
                    runtime, "_current_components", return_value=(component,)
                ),
                patch.object(
                    runtime,
                    "_git_snapshot_manifest_records",
            ...
            with contextlib.ExitStack() as stack:
                ...
                stack.enter_context(patch.object(runtime, "_git", side_effect=clean_git))
                readiness = runtime._current_candidate_readiness(repo)
            self.assertTrue(readiness["ready"])
            self.assertTrue(readiness["candidate_is_ancestor"])
            self.assertTrue(readiness["components"][0]["matched"])

            def stale_git(_repo: Path, *args: str) -> str:
                if args == ("rev-parse", "HEAD"):
                    return toolchain
                if args == ("rev-parse", f"{candidate}^{{commit}}"):
                    return candidate
                if args == ("status", "--short", "--untracked-files=all"):
                    return " M japan_expanded_v2/events/stale.txt"
                if args == ("merge-base", candidate, toolchain):
                    return "c" * 40
                if args[:4] == ("diff", "--name-only", "--no-renames", candidate):
                    return "japan_expanded_v2/events/stale.txt"
                self.fail(f"unexpected Git request: {args!r}")

            mismatched_manifest = live_manifest[:-1]
            with contextlib.ExitStack() as stack:
                ...
                stack.enter_context(
                    patch.object(
                        runtime,
                        "_git_snapshot_manifest_records",
                        return_value=mismatched_manifest,
                    )
                )
                stack.enter_context(patch.object(runtime, "_git", side_effect=stale_git))
                readiness = runtime._current_candidate_readiness(repo)
            self.assertFalse(readiness["ready"])
            self.assertFalse(readiness["repository_clean"])
            self.assertFalse(readiness["candidate_is_ancestor"])
            self.assertFalse(readiness["runtime_diff_clean"])
            self.assertFalse(readiness["components"][0]["matched"])

    def test_current_deploy_requires_clean_matching_matrix_candidate(self) -> None:
        ready = {
            "ready": True,
            "candidate_revision": "a" * 40,
        }
        with patch.object(
            runtime, "_current_candidate_readiness", return_value=ready
        ):
            self.assertEqual(ready, runtime._require_current_candidate_ready(LIVE_REPO))
        ...
        blocked = {
            ...
            "components": [{"matched": False}],
        }
        with patch.object(
            runtime, "_current_candidate_readiness", return_value=blocked
        ):
            with self.assertRaisesRegex(
                runtime.AcceptanceError, "not safe to deploy"
            ):
                runtime._require_current_candidate_ready(LIVE_REPO)

        with patch.object(runtime, "_current_revision", return_value="d" * 40):
            self.assertEqual("d" * 40, runtime._require_clean_control_plane(LIVE_REPO))
        with patch.object(
            runtime, "_current_revision", return_value="d" * 40 + "+dirty"
        ):
            with self.assertRaisesRegex(
                runtime.AcceptanceError, "clean committed control-plane"
            ):
                runtime._require_clean_control_plane(LIVE_REPO)

        with tempfile.TemporaryDirectory() as temporary:
            with _runtime_environment(Path(temporary)) as env:
                with patch.object(
                    runtime,
                    "_require_clean_control_plane",
                    side_effect=runtime.AcceptanceError("dirty control plane"),
                ):
                    with self.assertRaisesRegex(
                        runtime.AcceptanceError, "dirty control plane"
                    ):
                        runtime.deploy(
                            env.repo,
                            env.user,
                ...
                with (
                    ...
                    patch.object(
                        runtime,
                        "_require_current_candidate_ready",
                        return_value={
                            "ready": True,
                            "candidate_revision": "a" * 40,
                        },
                    ),
                    patch.object(runtime, "_git", return_value="b" * 40),
                ):
                    with self.assertRaisesRegex(
                        runtime.AcceptanceError,
                        "must use matrix candidate",
                    ):
                        runtime.deploy(
                            env.repo,
                            env.user,
    ...
    def test_session_binds_control_plane_until_collect_but_abort_survives_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            with _runtime_environment(Path(temporary)) as env:
                ...
                self.assertEqual("d" * 40, record["control_plane_revision"])

                with patch.object(
                    runtime, "_require_clean_control_plane", return_value="e" * 40
                ):
                    with self.assertRaisesRegex(
                        runtime.AcceptanceError,
                        "changed between before-session and collect",
                    ):
                        runtime.collect(
                            session,
                            [],
                            session / runtime.EVIDENCE_MANIFEST_NAME,
                        )
                self.assertTrue((env.user / runtime.ACTIVE_SESSION_NAME).is_file())
                with (
                    patch.object(
                        runtime,
                        "_current_candidate_revision",
                        side_effect=AssertionError(
                            "abort must not consult the current candidate"
                        ),
                    ),
                    patch.object(
                        runtime,
                        "_scenario",
                        side_effect=AssertionError(
                            "abort must not consult the current scenario matrix"
                        ),
                    ),
                    patch.object(
                        runtime,
                        "_scenario_matrix_sha256",
                        side_effect=AssertionError(
                            "abort must not consult the current matrix hash"
                        ),
                    ),
                    patch.object(runtime, "DAILY_VFS_COVERAGE", ("changed",)),
                ):
                    authenticated, _, _ = runtime._validated_collection_session_identity(
                        session, require_active=True
                    )
                    self.assertEqual(record["session_id"], authenticated["session_id"])
                    aborted = runtime.abort_session(
                        session, "test cleanup after matrix and control-plane drift"
                    )
    ...
    def test_matrix_schema2_has_independent_probe_and_exact_blocker_scope(self) -> None:
        ...
        self.assertEqual(set(), blocked_scenarios)
        r13 = runtime._normalized_evidence_contract(runtime._scenario("R13"), None)
        self.assertEqual(17, len(r13["parents"]))
        self.assertIn(
            "r4_028_pre_political_collection",
            {item["role"] for item in r13["parents"]},
        )

    def test_r9_requires_exact_exclusive_toyotomi_identity_evidence(self) -> None:
        r9 = runtime._scenario("R9")
        normalized = runtime._normalized_evidence_contract(r9, None)
        self.assertEqual(19, normalized["minimum_screenshots"])
        self.assertEqual(
            runtime.R9_TOYOTOMI_IDENTITY_CONTRACT,
            r9["toyotomi_identity_contract"],
        )
        for role in runtime.R9_TOYOTOMI_IDENTITY_CONTRACT[
            "identity_screenshot_roles"
        ]:
            self.assertEqual("screenshot", normalized["artifact_roles"][role])
        self.assertEqual(
            "checklist",
            normalized["artifact_roles"]["r9_toy_identity_checklist"],
        )
        self.assertEqual(
            "screenshot",
            normalized["artifact_roles"]["r9_1598_05_23_screenshot"],
        )
        identity_text = next(
            item["text"]
            for item in r9["acceptance"]
            if item["id"] == "r9_toyotomi_identity"
        )
        self.assertIn("exact five-series non-generic Toyotomi tree", identity_text)
        self.assertNotIn("six-node identity series", identity_text)

    def test_r9_rejects_restoring_the_oda_series_after_handoff(self) -> None:
        r9 = json.loads(json.dumps(runtime._scenario("R9"), ensure_ascii=False))
        r9["toyotomi_identity_contract"]["mission_series_by_slot"]["3"] = (
            "jxp_oda_toyotomi_house_missions"
        )
        with self.assertRaisesRegex(
            runtime.AcceptanceError, "R9 Toyotomi identity contract drifted"
        ):
            runtime._normalized_evidence_contract(r9, None)
    ...
    def test_matrix_contracts_close_assertions_roles_fixtures_and_todos(self) -> None:
        ...
        parent_assertion_count = sum(
            len(contract["assertions"])
            for contract in normalized_contracts
            if contract["scenario_id"] != "R13"
        )
        self.assertEqual(runtime._R13_PARENT_ASSERTION_COUNT, parent_assertion_count)
        covered = {
    ...
    def test_r5_external_contract_accepts_identity_and_rejects_tamper(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source, identity = _write_r5_text_eu4(root / "autosave.eu4")
            requirement = {
                "role": "r5_external_pre0242_input_save",
                "source": "external_original",
                "expected_filename": source.name,
                "expected_bytes": source.stat().st_size,
                "expected_sha256": runtime._sha256_file(source),
                "expected_identity": identity,
            }
            records = runtime._seal_input_saves(
                root / "session",
                (f"r5_external_pre0242_input_save={source}",),
                (requirement,),
            )
            self.assertEqual(identity, records[0]["external_identity"])

            payload = source.read_bytes().replace(
                b"date=1445.1.1", b"date=1446.1.1", 1
            )
            source.write_bytes(payload)
            tampered = dict(requirement)
            tampered["expected_bytes"] = source.stat().st_size
            tampered["expected_sha256"] = runtime._sha256_file(source)
            with self.assertRaises(runtime.AcceptanceError):
                runtime._seal_input_saves(
                    root / "tampered-session",
                    (f"r5_external_pre0242_input_save={source}",),
                    (tampered,),
                )

    def test_r5_matrix_pins_recovered_original_and_has_no_fixture_blocker(self) -> None:
        self.assertEqual(
            "recovered_pinned_external",
            runtime._scenario_matrix()["legacy_sources"]["pre-0.24.2"][
                "availability"
            ],
        )
        r5 = runtime._scenario("R5")
        normalized = runtime._normalized_evidence_contract(r5, None)
        requirement = normalized["inputs"][0]
        self.assertEqual("autosave.eu4", requirement["expected_filename"])
        self.assertEqual(36_371_037, requirement["expected_bytes"])
        self.assertEqual(
        ...
        self.assertEqual(
            [],
            runtime._normalized_evidence_contract(
                runtime._scenario("R13"), None
            )["blockers"],
        )

    def test_r5_escrow_verifier_binds_save_sidecars_and_backups(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary).resolve()
            source, identity = _write_r5_text_eu4(root / "autosave.eu4")
            required_fields = [
                "source_machine_or_custodian",
                "source_path_or_archive_name",
                "original_filename",
                "acquired_at_utc",
                "chain_of_custody",
                "size_bytes",
                "sha256",
                "eu4_version_if_known",
                "jxp_version_or_release_evidence",
                "original_failure_description",
            ...
            releases = {
                "before-one.zip": "0.23.0",
                "before-two.zip": "0.23.1",
            }
            backup_manifest: dict[str, dict[str, object]] = {}
            for filename, release in releases.items():
                backup = root / filename
                with zipfile.ZipFile(backup, "w", zipfile.ZIP_DEFLATED) as archive:
                    archive.writestr(
                        "japan_expanded_v2\\descriptor.mod",
                        f'name="fixture"\nversion="{release}"\n',
                    )
                backup_manifest[filename] = {
                    "bytes": backup.stat().st_size,
                    "sha256": runtime._sha256_file(backup),
                    "release": release,
                }
            timestamp = "2026-07-14T01:00:11Z"
            provenance = {
                "source_machine_or_custodian": "unit-test-custodian",
                "source_path_or_archive_name": str(source),
                "original_filename": source.name,
                "acquired_at_utc": timestamp,
                "chain_of_custody": ["created as a deterministic unit fixture"],
                "size_bytes": source.stat().st_size,
                "sha256": digest,
                "eu4_version_if_known": "EU4 1.37.5.0 Inca",
                "jxp_version_or_release_evidence": "pre-0.24.2 unit fixture",
                "original_failure_description": "serialized mission collision",
                "unaltered_bytes_attestation": True,
                "source_creation_time_utc": timestamp,
                "source_last_write_time_utc": timestamp,
                "escrow_path": str(source),
            }
            _write_json(root / "r5_external_input_provenance.json", provenance)
            _write(
                root / "r5_external_input_hash_manifest.txt",
                "\n".join(
                    (
                        ...
                        f"escrow_path={source}",
                        "source_and_escrow_hash_match=true",
                        "unaltered_bytes_attestation=true",
                        "",
                    )
                ),
            )
            version = identity["savegame_version"]
            ...
            content_identity = {
                ...
                "sha256": digest,
                "format": "EU4txt",
                "campaign_date": identity["date"],
                "player": identity["player"],
                "savegame_version": (
                    f"{version['first']}.{version['second']}.{version['third']}."
                    f"{version['forth']} {version['name']}"
                ),
                "enabled_mods": ["JXP unit fixture"],
                "required_player_flag": "jxp_path_open_trade",
                "player_completed_jxp_missions": identity["completed_jxp_missions"],
                "player_country_mission_slots": runtime._r5_sidecar_mission_slots(
                    identity["player_mission_slots"]
                ),
                "all_save_bom_prefixed_series_counts": identity["bom_series_counts"],
                "backup_release_bracket": releases,
            }
            identity_path = _write_json(
                root / "r5_content_identity.json", content_identity
            )
            result = runtime.verify_r5_escrow(
                root,
                requirement=requirement,
                backup_manifest=backup_manifest,
                expected_enabled_mods=("JXP unit fixture",),
            )
            self.assertEqual("ADMITTED_STATIC_INPUT", result["status"])
            self.assertFalse(result["game_started"])
            self.assertEqual(2, len(result["backups"]))

            tampered = dict(content_identity)
            tampered["campaign_date"] = "1446.1.1"
            _write_json(identity_path, tampered)
            with self.assertRaises(runtime.AcceptanceError):
                runtime.verify_r5_escrow(
                    root,
                    requirement=requirement,
                    backup_manifest=backup_manifest,
                    expected_enabled_mods=("JXP unit fixture",),
    ...
    def test_external_input_provenance_binds_original_size_hash_and_attestation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            original = _write_eu4(root / "autosave.eu4", "authentic")
            ...
            requirement["expected_bytes"] = size
            requirement["expected_sha256"] = digest
            normalized = dict(normalized)
            normalized["inputs"] = [requirement]
            provenance = {
                ...
                "sealed_inputs": [
                    {
                        ...
                        "bytes": size,
                        "sha256": digest,
                        "external_identity": requirement["expected_identity"],
                        "external_identity_sha256": runtime._object_sha256(
                            requirement["expected_identity"]
                        ),
                    }
                ]
            }
    ...
    def test_close_release_is_sealed_nonruntime_and_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            with _runtime_environment(Path(temporary)) as env:
                ...
                self.assertEqual(result, repeated)
                self.assertEqual("READY_FOR_LEAD_REVIEW", result["status"])
                self.assertEqual(17, len(result["parents"]))
    ...
    def test_r13_distribution_script_roots_cover_prefix_and_user_layouts(self) -> None:
        prefix_site = Path(r"C:\Python312\Lib\site-packages")
        user_site = Path(
            r"C:\Users\User\AppData\Roaming\Python\Python312\site-packages"
        )
        self.assertIn(
            Path(r"C:\Python312\Scripts"),
            runtime._r13_python_script_roots(prefix_site),
        )
        self.assertIn(
            Path(r"C:\Users\User\AppData\Roaming\Python\Python312\Scripts"),
            runtime._r13_python_script_roots(user_site),
        )
    ...
    def test_close_release_refuses_missing_parent_set_without_output(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            with _runtime_environment(Path(temporary)) as env:
                normalized = runtime._normalized_evidence_contract(
                    runtime._scenario("R13"), None
    ...
    def test_cli_has_observe_and_abort_but_no_launch_surface(self) -> None:
        ...
        self.assertEqual(
            runtime.DEFAULT_R5_ESCROW_ROOT,
            parser.parse_args(["verify-r5-escrow"]).escrow_root,
        )
