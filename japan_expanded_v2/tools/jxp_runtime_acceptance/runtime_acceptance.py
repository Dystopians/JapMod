...
CURRENT_SNAPSHOT_LABELS = frozenset({"current", "current-main"})
R9_TOYOTOMI_IDENTITY_CONTRACT = {
    "handoff_date": "1586.1.1",
    "predecessor_tag": "ODA",
    "successor_tag": "TOY",
    "ruler_name": "秀吉",
    "pre_handoff_dynasty": "羽柴",
    "post_handoff_dynasty": "丰臣",
    "idea_group": "TOY_ideas",
    "idea_slots": 7,
    "mission_series_by_slot": {
    ...
    "dedicated_missions_per_series": 7,
    "forbidden_post_handoff_series": ["jxp_oda_toyotomi_house_missions"],
    "founder_reform": "jxp_reform_founder_toyotomi_five_regents",
    "origin_flag": "jxp_origin_oda",
    "identity_checklist_role": "r9_toy_identity_checklist",
    "identity_screenshot_roles": [
        "r9_toy_ruler_name_screenshot",
        "r9_toy_ideas_screenshot",
        "r9_toy_missions_top_screenshot",
        "r9_toy_missions_middle_screenshot",
        "r9_toy_missions_bottom_screenshot",
        "r9_toy_government_screenshot",
        "r9_toy_subjects_screenshot",
    ],
    "boundary_expectations": {
        ...
        "1615.6.4": {
            "landed_tag": "TKG",
            "unlanded_tag": "ODA",
            "former_successor_unlanded_tag": "TOY",
        },
    },
}
...
BLOCKER_PATTERNS = {
    ...
    "government_reform_tier_overflow": re.compile(
        r"countryreformview\.cpp:\d+.*Trying to add more than 16 government "
        r"reforms to one tier",
        re.IGNORECASE,
    ),
    "unknown_text_property": re.compile(
        r"\bUnknown text property\b", re.IGNORECASE
    ),
    "country_name_pool": re.compile(
        r"countrydatabase\.cpp:\d+.*has less than 10 random "
        r"(?:leader|ship) names",
        re.IGNORECASE,
    ),
    "vanilla_otm_idea_override": re.compile(
        r"tag\s*=\s*OTM_ideas\b.*===>\s*Unite the Clans\b.*"
        r"\bTensho Embassy\b.*\bKunikuzushi\b",
        re.IGNORECASE,
    ),
    ...
    "map_initialization": re.compile(
        r"(?:"
        r"\[(?:map|province|adjacency|strait)[A-Za-z0-9_]*\.cpp:\d+\]"
        r".*\b(?:error|invalid|failed)\b|"
        r"\b(?:map (?:initialization|data)|province (?:id|definition|history)|"
        r"adjacency|strait)\b.{0,160}\b(?:error|invalid|failed)\b|"
        r"\b(?:error|invalid|failed)\b.{0,160}\b(?:map (?:initialization|data)|"
        r"province (?:id|definition|history)|adjacency|strait)\b"
        r")",
        re.IGNORECASE,
    ),
}
BENIGN_LOG_PATTERNS = (
    re.compile(
        r"\[province\.cpp:\d+\]: Failed to remove prov \d+ as core for "
        r"[A-Z0-9_]+, it hasn'?t got it!",
        re.IGNORECASE,
    ),
)
IDEA_GROUP_LOG_PATTERN = re.compile(
    r"\[ideas\.cpp:491\]: Idea Group #(\d+) tag = ([A-Za-z0-9_]+)\b"
)
EXPECTED_VISIBLE_IDEA_REGISTRY_IDS = frozenset(
    (*range(1, 26), *range(226, 514))
)
EXPECTED_VISIBLE_IDEA_REGISTRY_ANCHORS = {
    226: "LIV_ideas",
    482: "voc_ideas",
    483: "jxp_map_new_daimyo_ideas",
    484: "ANK_ideas",
    503: "RKK_ideas",
    513: "YMC_ideas",
}
CORE_REMOVE_LOG_PATTERN = re.compile(
    r"\[province\.cpp:\d+\]: Failed to remove prov (\d+) as core for "
    r"([A-Z0-9_]+), it hasn'?t got it!",
    re.IGNORECASE,
)
...
@lru_cache(maxsize=1)
def _jxp_owned_province_ids() -> frozenset[int]:
    owned: set[int] = set()
    for component in ("japan_expanded_v2", "japan_expanded_v2_map"):
        history_root = HELPER_REPO_ROOT / component / "history" / "provinces"
        if not history_root.exists():
            continue
        for path in history_root.glob("*.txt"):
...
# EU4 1.37.5 parses -userdir before normal Windows quote handling and can
# truncate a value that contains whitespace. Keep the reviewed acceptance
# root outside the whitespace-bearing profile path on this host.
DEFAULT_ACCEPTANCE_USER_DATA = Path(r"C:\JXP_Acceptance")
DEFAULT_R5_ESCROW_ROOT = Path(r"C:\JXP_R5_Escrow\20260713_original_machine")
R5_EXPECTED_ENABLED_MODS = (
    "Chinese Language Supplementary Mod for 1.37",
    "Chinese Language Mod for 1.37",
    "日轮诸道：日本扩展风味包",
)
R5_ESCROW_BACKUPS: dict[str, dict[str, object]] = {
    "japan_expanded_v2_before_mission_layout_hotfix_20260709_154614.zip": {
        "bytes": 47_308_072,
        "sha256": "64f11ace4f049b799b1c9d5cf0fa64d0a75abf485d90c0dbc0814fc71107485d",
        "release": "0.23.0",
    },
    "japan_expanded_v2_before_series_consolidation_20260709_161217.zip": {
        "bytes": 47_373_265,
        "sha256": "c05c1b9ee8d1aab31e02b472ae2c8f897cdfa08e07b33b75367eac1b3984ae3d",
        "release": "0.23.1",
    },
    "japan_expanded_v2_before_reform_visibility_hotfix_20260709_164900.zip": {
        "bytes": 48_071_664,
        "sha256": "cdbcfd3a7abe5653bc8c44a377912511c6ec864587d0a504aec21c949f6b6ae4",
        "release": "0.23.1",
    },
}
...
def _read_stable_utf8_text(path: Path, label: str, *, max_bytes: int) -> str:
    if not path.is_file() or _is_link_or_junction(path):
        raise AcceptanceError(f"{label} must be an ordinary file: {path}")
    before = path.stat()
...
def deploy(
    ...
    control_plane_revision = _require_clean_control_plane(repo)
    candidate_readiness: dict[str, object] | None = None
    if safe_label in CURRENT_SNAPSHOT_LABELS:
        candidate_readiness = _require_current_candidate_ready(repo)
        requested = revision or "HEAD"
        resolved_requested = _git(repo, "rev-parse", f"{requested}^{{commit}}")
        if resolved_requested != candidate_readiness["candidate_revision"]:
            raise AcceptanceError(
                f"{safe_label} deployment must use matrix candidate "
                f"{candidate_readiness['candidate_revision']}; resolved "
                f"{resolved_requested}"
            )
...
def _normalize_external_save_identity(value: object) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != {
        ...
        "savegame_version",
        "country_flags",
        "completed_jxp_missions",
        "player_mission_slots",
        "bom_series_counts",
    }:
        raise AcceptanceError("external save content-identity schema disagrees")
    if value.get("schema") != "jxp_r5_pre0242_identity/v1":
        raise AcceptanceError("external save content-identity version disagrees")
    if value.get("save_format") != "eu4text":
        raise AcceptanceError("R5 content identity requires an EU4txt save")
    if not isinstance(value.get("date"), str) or not re.fullmatch(
        r"\d{1,4}\.\d{1,2}\.\d{1,2}", str(value["date"])
    ):
        raise AcceptanceError("external save identity date is invalid")
    if not isinstance(value.get("player"), str) or not re.fullmatch(
        r"[A-Z0-9-]{3}", str(value["player"])
    ):
        raise AcceptanceError("external save identity player tag is invalid")

    version = value.get("savegame_version")
    if not isinstance(version, dict) or set(version) != {
        ...
        raise AcceptanceError("external save identity version schema disagrees")
    for key in ("first", "second", "third", "forth"):
        if isinstance(version.get(key), bool) or not isinstance(version.get(key), int):
            raise AcceptanceError(f"external save identity version {key} is invalid")
    if not isinstance(version.get("name"), str) or not version["name"].strip():
        raise AcceptanceError("external save identity version name is invalid")

    flags = value.get("country_flags")
    if (
        not isinstance(flags, dict)
        or not flags
        or not all(
            isinstance(key, str)
            and re.fullmatch(r"[a-z0-9_]{3,95}", key)
            and isinstance(flag_date, str)
            and re.fullmatch(r"\d{1,4}\.\d{1,2}\.\d{1,2}", flag_date)
            for key, flag_date in flags.items()
        )
    ):
        raise AcceptanceError("external save identity country flags are invalid")

    completed = value.get("completed_jxp_missions")
    if (
        not isinstance(completed, list)
        or not completed
        or len(completed) != len(set(completed))
        or not all(
            isinstance(item, str) and re.fullmatch(r"jxp_[a-z0-9_]{3,95}", item)
            for item in completed
        )
    ):
        raise AcceptanceError("external save identity completed missions are invalid")

    slots = value.get("player_mission_slots")
    if not isinstance(slots, list) or len(slots) != 5:
        raise AcceptanceError("external save identity must declare exactly five slots")
    for slot in slots:
        if (
            ...
            or not all(
                isinstance(item, str)
                and re.fullmatch(r"\ufeff?[A-Za-z0-9_]{3,95}", item)
                for item in slot
            )
        ):
            raise AcceptanceError("external save identity mission slots are invalid")

    bom_counts = value.get("bom_series_counts")
    if (
        not isinstance(bom_counts, dict)
        or not bom_counts
        or not all(
            isinstance(key, str)
            and re.fullmatch(r"jxp_[a-z0-9_]{3,95}", key)
            and not isinstance(count, bool)
            and isinstance(count, int)
            and count > 0
            for key, count in bom_counts.items()
        )
    ):
        raise AcceptanceError("external save identity BOM-series counts are invalid")
...
def _validate_r9_toyotomi_identity_contract(
    scenario: dict[str, object],
    artifact_roles: dict[str, str],
    assertion_ids: Sequence[str],
) -> None:
    """Keep R9 aligned with the exclusive five-column Toyotomi identity."""

    scenario_id = str(scenario.get("id", "")).upper()
    identity_contract = scenario.get("toyotomi_identity_contract")
    if scenario_id != "R9":
        if identity_contract is not None:
            raise AcceptanceError(
                "only R9 may declare the Toyotomi identity runtime contract"
            )
        return
    if identity_contract != R9_TOYOTOMI_IDENTITY_CONTRACT:
        raise AcceptanceError(
            "R9 Toyotomi identity contract drifted from the exact ODA/TOY handoff, "
            "name, ideas, missions, reform, origin, or boundary requirements"
        )
    if not {"r9_toyotomi_identity", "r9_history_boundaries"}.issubset(
        set(assertion_ids)
    ):
        raise AcceptanceError("R9 is missing a Toyotomi identity assertion")
    for role in R9_TOYOTOMI_IDENTITY_CONTRACT["identity_screenshot_roles"]:
        if artifact_roles.get(str(role)) != "screenshot":
            raise AcceptanceError(
                f"R9 Toyotomi identity screenshot role is missing or mistyped: {role}"
            )
    checklist_role = str(
        R9_TOYOTOMI_IDENTITY_CONTRACT["identity_checklist_role"]
    )
    if artifact_roles.get(checklist_role) != "checklist":
        raise AcceptanceError(
            "R9 Toyotomi identity checklist is missing or mistyped"
        )
...
def _normalized_evidence_contract(
    ...
    if matrix.get("schema") != 2:
        raise AcceptanceError("schema-5 sessions require runtime matrix schema 2")
    ...
    _validate_r9_toyotomi_identity_contract(
        scenario, artifact_roles, [str(item) for item in assertion_ids]
    )
    ...
    for item in raw_inputs:
        ...
        if item.get("source") == "external_original":
            ...
            if (
                ...
                or not expected_filename.lower().endswith(".eu4")
                or isinstance(expected_bytes, bool)
                or not isinstance(expected_bytes, int)
                or not MIN_EU4_SAVE_BYTES <= expected_bytes <= MAX_EU4_SAVE_BYTES
                or not isinstance(expected_sha256, str)
                or not re.fullmatch(r"[0-9a-f]{64}", expected_sha256)
            ):
                raise AcceptanceError("external input-save evidence role closure disagrees")
            normalized_item = dict(item)
            normalized_item["expected_identity"] = _normalize_external_save_identity(
                item.get("expected_identity")
            )
...
def _canonical_snapshot_order(
    ...
    # Keep the declared dependency readable: parent before companion. Critical
    # translation collisions are made byte-identical instead of relying on an
    # inferred list-direction rule.
    return sorted(
        snapshots,
...
def _current_candidate_runtime_pathspecs(repo: Path) -> tuple[str, ...]:
    paths: list[str] = []
    for component in _current_components(repo, False):
        component_root = component.source.relative_to(repo).as_posix()
        paths.extend(
            f"{component_root}/{directory}"
            for directory in RUNTIME_DIRECTORIES[component.key]
        )
        paths.extend(
            f"{component_root}/{filename}" for filename in RUNTIME_ROOT_FILES
        )
        paths.append(component.outer_descriptor.relative_to(repo).as_posix())
    return tuple(dict.fromkeys(paths))


def _current_candidate_readiness(repo: Path) -> dict[str, object]:
    """Prove that the clean control plane still represents its pinned payload."""

    repo = _require_canonical_repo(repo)
    candidate_revision = _current_candidate_revision()
    toolchain_revision = _git(repo, "rev-parse", "HEAD")
    resolved_candidate = _git(
        repo, "rev-parse", f"{candidate_revision}^{{commit}}"
    )
    if resolved_candidate != candidate_revision:
        raise AcceptanceError(
            "runtime matrix candidate must be an exact full Git commit id"
        )
    ...
    try:
        merge_base = _git(
            repo, "merge-base", candidate_revision, toolchain_revision
        )
    except AcceptanceError:
        merge_base = ""
    candidate_is_ancestor = merge_base == candidate_revision

    runtime_diff_paths = tuple(
        line
        for line in _git(
            repo,
            "diff",
            ...
            candidate_revision,
            "--",
            *_current_candidate_runtime_pathspecs(repo),
        ).splitlines()
        if line
    )
    ...
    for component in _current_components(repo, False):
        live_manifest = tuple(
            _normalized_manifest_record(item) for item in _manifest(component)
        )
        candidate_manifest = _git_snapshot_manifest_records(
            component.key, candidate_revision
        )
        live_manifest_object = [
            {"path": path, "bytes": size, "sha256": digest}
            for path, size, digest in live_manifest
        ]
        candidate_manifest_object = [
            {"path": path, "bytes": size, "sha256": digest}
            for path, size, digest in candidate_manifest
        ]
        component_records.append(
            {
                "component": component.key,
                "live_files": len(live_manifest),
                "candidate_files": len(candidate_manifest),
                "live_manifest_sha256": _object_sha256(live_manifest_object),
                "candidate_manifest_sha256": _object_sha256(
                    candidate_manifest_object
                ),
                "matched": live_manifest == candidate_manifest,
            }
        )

    repository_clean = not worktree_changes
    runtime_diff_clean = not runtime_diff_paths
    components_match = all(
        bool(item["matched"]) for item in component_records
    )
    return {
        "ready": repository_clean
        and candidate_is_ancestor
        and runtime_diff_clean
        ...
        "runtime_diff_paths": list(runtime_diff_paths),
        "components": component_records,
    }


def _require_current_candidate_ready(repo: Path) -> dict[str, object]:
    readiness = _current_candidate_readiness(repo)
    if readiness["ready"]:
        return readiness
    reasons: list[str] = []
    if not readiness["repository_clean"]:
        reasons.append("control-plane worktree is not clean")
    if not readiness["candidate_is_ancestor"]:
        reasons.append("candidate is not an ancestor of the control-plane HEAD")
    if not readiness["runtime_diff_clean"]:
        reasons.append("runtime paths differ from the matrix candidate")
    if not all(bool(item["matched"]) for item in readiness["components"]):
        reasons.append("live runtime manifests differ from the matrix candidate")
    raise AcceptanceError(
        "current candidate is not safe to deploy: " + "; ".join(reasons)
    )


def _require_clean_control_plane(repo: Path) -> str:
    revision = _current_revision(repo)
    if not re.fullmatch(r"[0-9a-f]{40}", revision):
        raise AcceptanceError(
            "deployment requires a clean committed control-plane revision; "
            f"found {revision!r}"
        )
    return revision
...
def _unique_clausewitz_block(
    payload: bytes, pattern: bytes, label: str
) -> bytes:
    matches = list(re.finditer(pattern, payload))
    if len(matches) != 1:
        raise AcceptanceError(
            f"R5 identity requires exactly one {label} block, found {len(matches)}"
        )
    match = matches[0]
    opening = payload.find(b"{", match.start(), match.end())
    if opening < 0:
        raise AcceptanceError(f"R5 identity {label} block has no opening brace")
    depth = 0
    quoted = False
    escaped = False
    comment = False
    for index in range(opening, len(payload)):
        ...
        if value == 35:
            ...
            if depth < 0:
                break
    raise AcceptanceError(f"R5 identity {label} block is unbalanced")


def _unique_clausewitz_scalar(
    payload: bytes, pattern: bytes, label: str
) -> bytes:
    values = re.findall(pattern, payload)
    if len(values) != 1 or not isinstance(values[0], bytes):
        raise AcceptanceError(
            f"R5 identity requires exactly one {label} value, found {len(values)}"
        )
    return values[0]


def _r5_text_save_identity(
    ...
    if any(
        ...
        raise AcceptanceError(f"R5 source save changed while reading: {path}")
    if not payload.startswith(b"EU4txt\n"):
        raise AcceptanceError("R5 content identity requires a full EU4txt save")

    date = _unique_clausewitz_scalar(
        payload, rb"(?m)^date=(\d{1,4}\.\d{1,2}\.\d{1,2})\r?$", "date"
    ).decode("ascii")
    player = _unique_clausewitz_scalar(
        payload, rb'(?m)^player="([A-Z0-9-]{3})"\r?$', "player"
    ).decode("ascii")
    version_block = _unique_clausewitz_block(
        payload, rb"(?m)^savegame_version=\{", "savegame_version"
    )
    version: dict[str, object] = {}
    for key in ("first", "second", "third", "forth"):
        version[key] = int(
            _unique_clausewitz_scalar(
                version_block,
                rf"(?m)^\s*{key}=(\d+)\r?$".encode("ascii"),
                f"savegame_version.{key}",
            )
        )
    version["name"] = _unique_clausewitz_scalar(
        version_block,
        rb'(?m)^\s*name="([^"\r\n]+)"\r?$',
        "savegame_version.name",
    ).decode("utf-8", "strict")

    countries = _unique_clausewitz_block(
        payload, rb"(?m)^countries=\{", "countries"
    )
    player_block = _unique_clausewitz_block(
        countries,
        rb"(?m)^\t" + re.escape(player.encode("ascii")) + rb"=\{",
        f"player country {player}",
    )
    flags_block = _unique_clausewitz_block(
        player_block, rb"(?m)^\t\tflags=\{", "player flags"
    )
    expected_flags = expected["country_flags"]
    if not isinstance(expected_flags, dict):
        raise AcceptanceError("R5 expected country flags are malformed")
    observed_flags: dict[str, str] = {}
    for key in expected_flags:
        raw_value = _unique_clausewitz_scalar(
            flags_block,
            rb"(?m)^\s*"
            + re.escape(str(key).encode("ascii"))
            + rb"=(\d{1,4}\.\d{1,2}\.\d{1,2})\r?$",
            f"player flag {key}",
        )
        observed_flags[str(key)] = raw_value.decode("ascii")

    completed_block = _unique_clausewitz_block(
        player_block,
        rb"(?m)^\t\tcompleted_missions=\{",
        "player completed_missions",
    )
    completed = [
        value.decode("ascii")
        for value in re.findall(rb'"(jxp_[a-z0-9_]+)"', completed_block)
    ]

    missions_block = _unique_clausewitz_block(
        player_block,
        rb"(?m)^\t\tcountry_missions=\{",
        "player country_missions",
    )
    slot_matches = list(re.finditer(rb"(?m)^mission_slot=\{", missions_block))
    if len(slot_matches) != 5:
        raise AcceptanceError(
            f"R5 identity requires five player mission slots, found {len(slot_matches)}"
        )
    slots: list[list[str]] = []
    for index, match in enumerate(slot_matches, start=1):
        next_start = (
            slot_matches[index].start()
            if index < len(slot_matches)
            else len(missions_block)
        )
        slot_source = missions_block[match.start() : next_start]
        slot_block = _unique_clausewitz_block(
            slot_source,
            rb"(?m)^mission_slot=\{",
            f"player mission slot {index}",
        )
        opening = slot_block.find(b"{")
        try:
            slots.append(
                [token.decode("utf-8", "strict") for token in slot_block[opening + 1 : -1].split()]
            )
        except UnicodeDecodeError as exc:
            raise AcceptanceError(
                f"R5 player mission slot {index} is not valid UTF-8"
            ) from exc

    expected_counts = expected["bom_series_counts"]
    if not isinstance(expected_counts, dict):
        raise AcceptanceError("R5 expected BOM-series counts are malformed")
    bom_counts = {
        str(name): payload.count(b"\xef\xbb\xbf" + str(name).encode("ascii"))
        for name in expected_counts
    }
    return {
        "schema": "jxp_r5_pre0242_identity/v1",
        "save_format": "eu4text",
        "date": date,
        "player": player,
        "savegame_version": version,
        "country_flags": observed_flags,
        "completed_jxp_missions": completed,
        "player_mission_slots": slots,
        "bom_series_counts": bom_counts,
    }


def _validate_external_original_input(
    path: Path,
    ...
    if (
        path.name != requirement.get("expected_filename")
        or path.stat().st_size != requirement.get("expected_bytes")
        or digest != requirement.get("expected_sha256")
    ):
        raise AcceptanceError(
            "external original filename, size, or SHA-256 disagrees with the "
            f"reviewed matrix contract: {path}"
        )
    expected_identity = requirement.get("expected_identity")
    if not isinstance(expected_identity, dict):
        raise AcceptanceError("external original has no normalized identity contract")
    if validation.get("format") != expected_identity.get("save_format"):
        raise AcceptanceError("external original save format disagrees with identity")
    observed_identity = _r5_text_save_identity(path, expected_identity)
    if not _json_exact_equal(observed_identity, expected_identity):
        raise AcceptanceError(
            "external original content identity disagrees with the reviewed R5 contract"
        )
    return observed_identity


def _r5_external_requirement() -> dict[str, object]:
    normalized = _normalized_evidence_contract(_scenario("R5"), None)
    requirements = [
        item
        for item in normalized["inputs"]
        if item.get("source") == "external_original"
    ]
    if len(requirements) != 1 or normalized["blockers"]:
        raise AcceptanceError(
            "R5 matrix must expose one admitted external original and no blocker"
        )
    return requirements[0]


def _parse_r5_hash_manifest(path: Path) -> dict[str, str]:
    text = _read_stable_utf8_text(
        path, "R5 external hash manifest", max_bytes=16 * 1024
    )
    result: dict[str, str] = {}
    for line in text.splitlines():
        if not line:
            continue
        if "=" not in line:
            raise AcceptanceError(f"R5 hash manifest line is malformed: {line!r}")
        key, value = line.split("=", 1)
        if not re.fullmatch(r"[a-z][a-z0-9_]{2,63}", key) or not value:
            raise AcceptanceError(f"R5 hash manifest line is malformed: {line!r}")
        if key in result:
            raise AcceptanceError(f"R5 hash manifest repeats key {key!r}")
        result[key] = value
    expected_keys = {
        "original_filename",
        "size_bytes",
        ...
        "source_path",
        "escrow_path",
        "source_and_escrow_hash_match",
        "unaltered_bytes_attestation",
    }
    if set(result) != expected_keys:
        raise AcceptanceError("R5 hash manifest key set disagrees")
    return result


def _r5_sidecar_mission_slots(
    slots: object,
) -> dict[str, list[str]]:
    if not isinstance(slots, list):
        raise AcceptanceError("R5 matrix mission slots are malformed")
    return {
        str(index): [
            f"U+FEFF{item[1:]}" if item.startswith("\ufeff") else item
            for item in slot
        ]
...
def _r5_backup_record(path: Path, expected: dict[str, object]) -> dict[str, object]:
    if not path.is_file() or _is_link_or_junction(path):
        raise AcceptanceError(f"R5 backup must be an ordinary file: {path}")
    if (
        path.stat().st_size != expected["bytes"]
        or _sha256_file(path) != expected["sha256"]
    ):
        raise AcceptanceError(f"R5 backup size or SHA-256 disagrees: {path.name}")
    try:
        with zipfile.ZipFile(path) as archive:
            descriptor_members = [
                item
                for item in archive.infolist()
                if item.filename.replace("\\", "/").endswith("/descriptor.mod")
            ]
            if len(descriptor_members) != 1:
                raise AcceptanceError(
                    f"R5 backup must contain one nested descriptor.mod: {path.name}"
                )
            member = descriptor_members[0]
            if member.flag_bits & 0x1 or member.file_size > 64 * 1024:
                raise AcceptanceError(f"R5 backup descriptor is unsafe: {path.name}")
            descriptor = archive.read(member).decode("utf-8-sig", errors="strict")
    except (OSError, UnicodeError, zipfile.BadZipFile, RuntimeError) as exc:
        raise AcceptanceError(f"cannot validate R5 backup {path}: {exc}") from exc
    versions = re.findall(r'(?m)^version="([^"\r\n]+)"\r?$', descriptor)
    if versions != [expected["release"]]:
        raise AcceptanceError(f"R5 backup descriptor version disagrees: {path.name}")
    return {
        "filename": path.name,
...
def verify_r5_escrow(
    escrow_root: Path,
    *,
    requirement: dict[str, object] | None = None,
    backup_manifest: dict[str, dict[str, object]] | None = None,
    expected_enabled_mods: Sequence[str] = R5_EXPECTED_ENABLED_MODS,
) -> dict[str, object]:
    """Read-only verification of the authentic R5 save and every escrow sidecar."""

    root = _require_ordinary_directory(escrow_root, "R5 external escrow root")
    requirement = requirement or _r5_external_requirement()
    backups = backup_manifest or R5_ESCROW_BACKUPS
    expected_filename = requirement.get("expected_filename")
    if not isinstance(expected_filename, str):
        raise AcceptanceError("R5 external requirement has no expected filename")
    save = root / expected_filename
    if save.parent != root:
        raise AcceptanceError("R5 expected filename escaped the escrow root")
    validation = _validate_eu4_save(save)
    save_sha256 = _sha256_file(save)
    observed_identity = _validate_external_original_input(
        save, validation, save_sha256, requirement
    )

    provenance_path = root / "r5_external_input_provenance.json"
    hash_path = root / "r5_external_input_hash_manifest.txt"
    identity_path = root / "r5_content_identity.json"
    provenance = _read_json_object(
        provenance_path, "R5 external provenance", max_bytes=64 * 1024
    )
    required_fields = set(
        str(item) for item in requirement.get("required_provenance_fields", ())
    )
    optional_forensic_fields = {
        "source_creation_time_utc",
        "source_last_write_time_utc",
        "escrow_path",
    }
    if set(provenance) != required_fields | optional_forensic_fields:
        raise AcceptanceError("R5 provenance field set disagrees")
    if any(
        not _populated_provenance_value(provenance.get(field))
        for field in required_fields - {"unaltered_bytes_attestation"}
    ):
        raise AcceptanceError("R5 provenance contains an empty required field")
    expected_bytes = requirement.get("expected_bytes")
    if (
        provenance.get("original_filename") != expected_filename
        or provenance.get("size_bytes") != expected_bytes
        or provenance.get("sha256") != save_sha256
        or provenance.get("unaltered_bytes_attestation") is not True
        or provenance.get("escrow_path") != str(save)
        or Path(str(provenance.get("source_path_or_archive_name"))).name
        != expected_filename
    ):
        raise AcceptanceError("R5 provenance identity or attestation disagrees")
    chain = provenance.get("chain_of_custody")
    if (
        not isinstance(chain, list)
        or not chain
        or not all(isinstance(item, str) and item.strip() for item in chain)
    ):
        raise AcceptanceError("R5 provenance chain of custody is malformed")
    for field in (
        "acquired_at_utc",
        "source_creation_time_utc",
        "source_last_write_time_utc",
    ):
        _aware_datetime(provenance[field], f"R5 provenance {field}")

    hash_manifest = _parse_r5_hash_manifest(hash_path)
    if (
        hash_manifest["original_filename"] != expected_filename
        or hash_manifest["size_bytes"] != str(expected_bytes)
        or hash_manifest["sha256"] != save_sha256
        or hash_manifest["source_path"]
        != provenance["source_path_or_archive_name"]
        or hash_manifest["escrow_path"] != str(save)
        or hash_manifest["source_and_escrow_hash_match"] != "true"
        or hash_manifest["unaltered_bytes_attestation"] != "true"
    ):
        raise AcceptanceError("R5 external hash manifest disagrees")

    sidecar_identity = _read_json_object(
        identity_path, "R5 content identity", max_bytes=128 * 1024
    )
    expected_identity_keys = {
        ...
        "savegame_version",
        "enabled_mods",
        "required_player_flag",
        "player_completed_jxp_missions",
        "player_country_mission_slots",
        "all_save_bom_prefixed_series_counts",
        "backup_release_bracket",
    }
    expected_content = requirement.get("expected_identity")
    if not isinstance(expected_content, dict) or set(sidecar_identity) != expected_identity_keys:
        raise AcceptanceError("R5 content-identity sidecar schema disagrees")
    version = expected_content["savegame_version"]
    if not isinstance(version, dict):
        raise AcceptanceError("R5 matrix save version is malformed")
    expected_version = (
        f"{version['first']}.{version['second']}.{version['third']}."
        f"{version['forth']} {version['name']}"
    )
    expected_sidecar = {
        "schema": "jxp_r5_authentic_input_identity/v1",
        "original_filename": expected_filename,
        "size_bytes": expected_bytes,
        "sha256": save_sha256,
        "format": "EU4txt",
        "campaign_date": expected_content["date"],
        "player": expected_content["player"],
        "savegame_version": expected_version,
        "enabled_mods": list(expected_enabled_mods),
        "required_player_flag": next(iter(expected_content["country_flags"])),
        "player_completed_jxp_missions": expected_content["completed_jxp_missions"],
        "player_country_mission_slots": _r5_sidecar_mission_slots(
            expected_content["player_mission_slots"]
        ),
        "all_save_bom_prefixed_series_counts": expected_content["bom_series_counts"],
        "backup_release_bracket": {
            filename: spec["release"] for filename, spec in backups.items()
        },
    }
    if not _json_exact_equal(sidecar_identity, expected_sidecar):
        raise AcceptanceError("R5 content-identity sidecar disagrees with the matrix")

    backup_records = [
        _r5_backup_record(root / filename, spec)
        for filename, spec in sorted(backups.items())
    ]
    return {
        "status": "ADMITTED_STATIC_INPUT",
        "game_started": False,
        ...
        "save": {
            ...
            "content_identity_sha256": _object_sha256(observed_identity),
        },
        "sidecars": {
            "provenance": _file_record(provenance_path),
            "hash_manifest": _file_record(hash_path),
            "content_identity": _file_record(identity_path),
        },
...
def _seal_input_saves(
    ...
    if requirements and set(parsed) != set(requirements_by_role):
        raise AcceptanceError(
            "provided input-save roles do not exactly match the normalized contract"
        )
    ...
    for role, raw_path in sorted(parsed.items()):
        ...
        if requirement is not None and requirement.get("source") == "external_original":
            external_identity = _validate_external_original_input(
                source, validation, digest, requirement
            )
        ...
        if external_identity is not None:
            record["external_identity"] = external_identity
            record["external_identity_sha256"] = _object_sha256(external_identity)
...
def before_session(
    ...
    control_plane_revision = _require_clean_control_plane(repo)
    candidate_readiness = _require_current_candidate_ready(repo)
    if candidate_revision != candidate_readiness["candidate_revision"]:
        raise AcceptanceError(
            "before-session candidate differs from the clean control-plane seal"
        )
    ...
    try:
        ...
        sealed_inputs = _seal_input_saves(
            session, input_saves, normalized["inputs"]
        )
...
def _scan_logs(paths: Iterable[Path]) -> list[dict[str, object]]:
    ...
    for path in paths:
        ...
        for number, line in enumerate(lines, 1):
            core_remove = CORE_REMOVE_LOG_PATTERN.search(line)
            if (
                core_remove is not None
                and int(core_remove.group(1)) in _jxp_owned_province_ids()
            ):
                matches.append(
                    {
                        "code": "jxp_owned_province_core_remove",
                        "file": path.name,
                        "line": number,
                        "text": line[:500],
                    }
                ...
                if len(matches) >= 500:
                    return matches
                continue
            if any(pattern.search(line) for pattern in BENIGN_LOG_PATTERNS):
                continue
            for code, pattern in BLOCKER_PATTERNS.items():
                if pattern.search(line):
                    ...
                    if len(matches) >= 500:
                        return matches
        if path.name.casefold() == "setup.log":
            initialized = any("IdeaDatabase Initialized" in line for line in lines)
            if not initialized:
                matches.append(
                    {
                        "code": "idea_database_not_initialized",
                        "file": path.name,
                        "line": 1,
                        "text": "setup.log does not contain IdeaDatabase Initialized",
                    }
                )
                continue
            idea_records = [
                ...
                for line in lines
                if (match := IDEA_GROUP_LOG_PATTERN.search(line)) is not None
            ]
            id_counts = Counter(group_id for group_id, _ in idea_records)
            tag_counts = Counter(tag for _, tag in idea_records)
            visible_ids = set(id_counts)
            missing_ids = sorted(EXPECTED_VISIBLE_IDEA_REGISTRY_IDS - visible_ids)
            extra_ids = sorted(visible_ids - EXPECTED_VISIBLE_IDEA_REGISTRY_IDS)
            duplicate_ids = sorted(group_id for group_id, count in id_counts.items() if count > 1)
            duplicate_tags = sorted(tag for tag, count in tag_counts.items() if count > 1)
            if missing_ids or extra_ids:
                matches.append(
                    {
                        "code": "idea_registry_visible_shape",
                        "file": path.name,
                        "line": 1,
                        "text": (
                            "setup.log visible IdeaDatabase IDs differ from the pinned "
                            f"1..25 + 226..513 shape; missing={missing_ids[:20]} "
            ...
            if duplicate_ids:
                matches.append(
                    {
                        "code": "idea_registry_duplicate_id",
                        "file": path.name,
                        "line": 1,
                        "text": "duplicate visible national idea IDs: "
            ...
            if duplicate_tags:
                matches.append(
                    {
                        "code": "idea_registry_duplicate_group",
                        "file": path.name,
                        "line": 1,
                        "text": "duplicate visible national idea groups: "
                        + ", ".join(duplicate_tags),
                    }
                )
            by_id = {group_id: tag for group_id, tag in idea_records}
            bad_anchors = {
                group_id: (by_id.get(group_id), expected)
                for group_id, expected in EXPECTED_VISIBLE_IDEA_REGISTRY_ANCHORS.items()
                if by_id.get(group_id) != expected
            }
            if bad_anchors:
                matches.append(
                    {
                        "code": "idea_registry_anchor_mismatch",
                        "file": path.name,
                        "line": 1,
                        "text": f"visible IdeaDatabase anchor mismatch: {bad_anchors}",
                    }
                )
            if len(matches) >= 500:
                return matches[:500]
...
def _validate_session_record(
    ...
    if (
        ...
        raise AcceptanceError("session scenario is missing or not runnable")
    if require_current and scenario != _scenario(str(scenario.get("id"))):
        raise AcceptanceError("session scenario is stale")
    candidate_revision = record["candidate_revision"]
    if (
        not isinstance(candidate_revision, str)
        or not re.fullmatch(r"[0-9a-f]{40}", candidate_revision)
    ):
        raise AcceptanceError("session candidate revision is invalid")
    if require_current and candidate_revision != _current_candidate_revision():
        raise AcceptanceError("session candidate revision is stale")
    control_plane_revision = record["control_plane_revision"]
    if not isinstance(control_plane_revision, str) or not re.fullmatch(
        r"[0-9a-f]{40}", control_plane_revision
    ):
        raise AcceptanceError("session control-plane revision is missing or invalid")
    ...
    if stored_oracle is not None and (
        ...
        or not isinstance(stored_oracle.get("contract"), dict)
    ...
    if require_current:
        ...
        if selected_phase != phase:
            raise AcceptanceError(
                "session phase no longer matches its scenario contract"
            )
        contract_identity, contract_sha256 = _scenario_contract_identity(
            scenario, phase
        )
    ...
    if (
        ...
        or not isinstance(record["contract_identity"], dict)
        ...
        or not isinstance(record["daily_vfs_coverage"], list)
        or (
            require_current
            and record["daily_vfs_coverage"] != list(DAILY_VFS_COVERAGE)
        )
...
def _external_input_evidence_checks(
    ...
    for requirement in normalized["inputs"]:
        ...
        if (
            ...
            != requirement.get("expected_filename")
            or expected_bytes != requirement.get("expected_bytes")
            or expected_sha256 != requirement.get("expected_sha256")
            or not _json_exact_equal(
                sealed.get("external_identity"), expected_identity
            )
            or sealed.get("external_identity_sha256")
            != _object_sha256(expected_identity)
            or isinstance(provenance.get("size_bytes"), bool)
            or provenance.get("size_bytes") != expected_bytes
        ...
        results.append(
            {
                ...
                "content_identity_sha256": sealed["external_identity_sha256"],
...
def _collect_schema4(
    ...
    control_plane_after = _require_clean_control_plane(repo)
    if control_plane_after != record["control_plane_revision"]:
        raise AcceptanceError(
            "control-plane revision changed between before-session and collect"
        )
    candidate_readiness = _require_current_candidate_ready(repo)
    if candidate_readiness["candidate_revision"] != record["candidate_revision"]:
        raise AcceptanceError(
            "candidate seal changed between before-session and collect"
        )
    daily_user_data = _require_canonical_daily_user_data(
    ...
    for role, kind_value in normalized["artifact_roles"].items():
        ...
        if kind == "log":
            if role.endswith("error_log"):
                ...
                log_name = "game.log"
            elif role.endswith("setup_log"):
                log_name = "setup.log"
            else:
                raise AcceptanceError(
                    "automatic log role does not identify error.log, game.log, "
                    f"or setup.log: {role}"
                )
...
_R13_PARENT_ASSERTION_COUNT = 72
...
_R13_TRUSTED_GIT = Path(r"C:\Program Files\Git\mingw64\bin\git.exe")
...
def _r13_python_distribution_manifest(
    ...
    records: list[dict[str, object]] = []
    # User-site layouts place Scripts beside site-packages; prefix installs
    # place it beside Lib. Console entry points are not copied into the
    # isolated import site in either layout.
    script_roots = _r13_python_script_roots(site)
    for relative in sorted(selected.files, key=lambda item: str(item).casefold()):
        ...
        if path.is_file() and any(
            _is_relative_to(path, scripts) for scripts in script_roots
        ):
...
def _r13_python_script_roots(site: Path) -> set[Path]:
    return {
        (site.parent / "Scripts").resolve(),
        (site.parent.parent / "Scripts").resolve(),
    }
...
def _r13_assertion_result_rows(
    ...
    if len(rows) != _R13_PARENT_ASSERTION_COUNT:
        raise AcceptanceError(
            "R13 must close exactly "
            f"{_R13_PARENT_ASSERTION_COUNT} PROBE/R1-R12 assertions; found {len(rows)}"
        )
    return rows
...
def _r13_lineage_rows(
    ...
    if len(rows) != 17:
        raise AcceptanceError("R13 lineage index must contain seventeen parents")
    return rows
...
def _closure_automated_check_results(
    ...
    results = [
        {
            "id": "seventeen_parent_lineages_semantic",
            "matched": len(parents) == 17
            and all(
...
def close_release(
    ...
    if (
        ...
        or len(normalized.get("parents", [])) != 17
    ...
    if set(parsed_parents) != expected_parent_roles:
        raise AcceptanceError(
            "--parent-collection roles do not exactly match the seventeen R13 parents"
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    verify_r5_parser = subparsers.add_parser("verify-r5-escrow")
    verify_r5_parser.add_argument(
        "--escrow-root", type=Path, default=DEFAULT_R5_ESCROW_ROOT
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "verify-r5-escrow":
            result = verify_r5_escrow(args.escrow_root)
            success = True
