...
def read_runtime_directory(relative: str) -> str:
    directory = MOD_ROOT.joinpath(*relative.split("/"))
    return "\n".join(
...
def main():
    ...
    mission_text = read_runtime_directory("missions")
    base_assignment_mission_text = "\n".join(
        (MOD_ROOT / "missions" / filename).read_text(encoding="cp1252")
        for filename in (
            "jxp_map_new_daimyo_missions.txt",
            "jxp_map_15_major_daimyo_identity_missions.txt",
        )
    )
    event_text = read_runtime_directory("events")
    ...
    modifier_text = read_runtime_directory("common/event_modifiers")
    effect_text = read_runtime_directory("common/scripted_effects")
    ...
    # Only the base new-daimyo assignment file owns the one-tag/one-tree
    # contract.  Later house-depth and major-daimyo overlays may legitimately
    # mention the same tags in their own potentials.
    mission_tags = re.findall(r"\btag\s*=\s*([A-Z0-9]{3})", base_assignment_mission_text)
    ...
    expected_counts = {"missionGroups": 53, "missions": 292, "decisions": 27, "events": 203}
