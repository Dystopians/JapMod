...
def core_actions(fragment: str) -> list[tuple[str, str]]:
    return re.findall(r"\b(add_core|remove_core)\s*=\s*([A-Z0-9_]+)", fragment)


def apply_core_actions(cores: set[str], actions: list[tuple[str, str]]) -> None:
...
def existing_transition_blocks(
    source_text: str,
    timeline: dict,
    dated_effects: list[list[str]] | None = None,
) -> tuple[bool, str]:
    """Plan ownership overlays without duplicating inherited vanilla core history.

    Existing provinces keep every source core action. The overlay only gives a
    core to a genuinely new owner that lacks one, removes cores introduced by
    an earlier overlay, and applies the explicit ODA/TOY/TKG handoff contract.
    """

    ranges = list(dated_block_ranges(source_text))
    prefix_end = ranges[0][1] if ranges else len(source_text)
    cores: set[str] = set()
    apply_core_actions(cores, core_actions(source_text[:prefix_end]))
    source_events = [
        (changed, index, core_actions(source_text[start:end]))
        for index, (changed, start, end) in enumerate(ranges)
    ]
    source_events.sort(key=lambda item: (item[0], item[1]))

    previous = timeline["root"]
    root_needs_core = previous not in cores
    synthetic_cores: set[str] = set()
    if root_needs_core:
        cores.add(previous)
        synthetic_cores.add(previous)

    source_cursor = 0
    entries = []
    for date, owner in timeline.get("changes", []):
        changed = date_tuple(date)
        while source_cursor < len(source_events) and source_events[source_cursor][0] <= changed:
            apply_core_actions(cores, source_events[source_cursor][2])
            source_cursor += 1

        effects = [f"owner = {owner}", f"controller = {owner}"]
        if owner not in cores:
            effects.append(f"add_core = {owner}")
            cores.add(owner)
            synthetic_cores.add(owner)

        explicit_toyotomi_handoff = owner == "TOY" or previous == "TOY"
        if (
            previous != owner
            and previous in cores
            and (previous in synthetic_cores or explicit_toyotomi_handoff)
        ):
            effects.append(f"remove_core = {previous}")
            cores.remove(previous)
            synthetic_cores.discard(previous)

        entries.append((changed, date, effects))
        previous = owner
...
def generate_existing_history(source_text: str, pid: int, dev: dict, history_plan: dict) -> str:
    ...
    if timeline:
        ...
        root_needs_core, blocks = existing_transition_blocks(
            text,
            timeline,
            history_plan.get("dated_effects", {}).get(str(pid)),
        )
        text = scrub_ownership(text)
        owner_header = f"owner = {timeline['root']}\ncontroller = {timeline['root']}\n"
        if root_needs_core:
            owner_header += f"add_core = {timeline['root']}\n"
