...
def load_gameplay_encoder() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "_jxp_map_gameplay_name_encoder", GAMEPLAY_ENCODER
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load gameplay-name encoder: {GAMEPLAY_ENCODER}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if not callable(getattr(module, "encode_gameplay_text", None)):
        raise RuntimeError("Gameplay-name encoder lacks encode_gameplay_text")
    return module
...
def country_definition(country: dict, color: list[int]) -> str:
    ...
    ship_names = map_ship_names(country)
    ship_name_rows = "\n".join(
        "\t" + " ".join(f'\"{name}\"' for name in ship_names[index:index + 4])
        for index in range(0, len(ship_names), 4)
    )
    r, g, b = color
...
def map_ship_names(country: dict) -> tuple[str, ...]:
    """Return a country-owned ASCII pool that is unique across the map VFS."""

    stem = re.sub(r"[^A-Za-z0-9]", "", str(country["name"]))
    if not stem:
        stem = str(country["tag"])
    return tuple(f"{stem}{suffix}Maru" for suffix in MAP_SHIP_NAME_SUFFIXES)
...
religion = {country.get('religion', 'shinto')}
technology_group = chinese
capital = {country['capital']}

{country['start']} = {{
	set_country_flag = jxp_map_origin_{country['tag'].lower()}
}}
        ...
        lines.extend(
            [
                ...
                *_render_modifier_block(archetype["bonus"], "\t\t"),
                ...
                f"\t\thas_country_flag = jxp_map_origin_{tag.lower()}",
                "\t}",
                "\tfree = yes",
                "",
            ]
...
def main() -> int:
    ...
    ship_names = [name for country in countries for name in map_ship_names(country)]
    if len(ship_names) != len({name.casefold() for name in ship_names}):
        raise RuntimeError("Generated map ship names must be globally unique")
    ...
    gameplay_encoder = load_gameplay_encoder()
    ...
    for index, country in enumerate(countries):
        ...
        (country_dir / filename).write_bytes(
            gameplay_encoder.encode_gameplay_text(
                country_definition(country, colors[tag])
            )
        )
        (history_dir / f"{tag} - {country['name']}.txt").write_bytes(
            gameplay_encoder.encode_gameplay_text(country_history(country, index))
        )
    ...
    idea_source_dir = (
        args.main_mod.resolve()
        / "tools"
        / "jxp_validation"
        / "idea_sources"
    )
    idea_source_dir.mkdir(parents=True, exist_ok=True)
    (idea_source_dir / "jxp_map_daimyo_ideas.txt").write_text(
        ideas_file(records), encoding="utf-8", newline="\n"
    )
    stale_runtime_ideas = MOD_ROOT / "common" / "ideas" / "jxp_map_new_daimyo_ideas.txt"
    if stale_runtime_ideas.exists():
        stale_runtime_ideas.unlink()
    ...
    manifest = {
        ...
        "ship_name_count_per_country": len(MAP_SHIP_NAME_SUFFIXES),
        "ship_name_count_global": sum(len(map_ship_names(country)) for country in countries),
        "ship_name_distinct_global": len(
            {
                name.casefold()
                for country in countries
                for name in map_ship_names(country)
            }
        ),
        "colors": colors,
