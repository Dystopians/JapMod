...
This redirect is a new user-data profile, not a read-only overlay on the normal
Documents profile. Missing language, display, audio, launcher, and gameplay
preferences are therefore expected on first start. Prove mod loading from fresh
isolated `setup.log`/`error.log` content and the visible main menu, not from the
presence of familiar preferences. During a main-menu-only probe, never buffer a
keyboard confirmation while the first-run language screen is still loading. A
queued `Enter` can propagate into the country-selection screen and start a
campaign, invalidating the probe. Select the visible language control only after
confirming focus, wait for the main menu, record the attestation, and send no
further input.
...
A runtime whitelist and a user-facing review bundle are different release
surfaces. An atomic payload checkout should still exclude `tools`, source art,
tests, and generated review pages from the gameplay identity. If the user opens
a visualizer from inside the deployed Mod folder, copy a fixed review-artifact
manifest from the clean control plane after the runtime swap, compare every
byte or SHA-256, and audit those files separately from runtime files. This keeps
the payload fingerprint immutable while preventing a runtime-only replacement
from deleting a previously valid local review URL.

If the acceptance matrix stores the candidate Git OID, do not try to make one
commit point to itself. First create payload commit A with the complete runtime
whitelist. Then, without changing any runtime byte, create control-plane commit
B that pins A in the matrix and refreshes oracle, ledger, manuals, and tests.
Run deployment and evidence tooling from clean B while materializing A. Require
the complete live runtime tree to be tracked and `git diff --quiet A --` every
runtime root to succeed. Production preflight must report not-ready when B is
dirty, A is not an ancestor of B, or either component manifest differs; current
deployment must repeat the check and require explicit revision A rather than
defaulting to HEAD/B. A runtime change after A invalidates the candidate and
requires a new A/B pair; documentation or tests excluded from runtime may change
in B.

Seal each runtime session against both halves of that pair. `before-session`
must require a clean B, re-prove that A is the ready candidate, and store B's
exact OID in the session record. Before accepting any evidence, `collect` must
again require a clean repository at the same B and re-prove the same A; a docs,
matrix, oracle, test, or ledger commit made during the run invalidates collection
even when the runtime whitelist is unchanged. Do not make abort depend on the
live repository still being at B: it must authenticate the immutable session and
release its nonce lock after drift, while leaving that run unusable as evidence.
Older session schemas remain historical records and must be aborted or archived,
never upgraded in place or relabelled as current evidence.
...
There is one byte-level exception for Chinese community playsets that use the
EU4 double-byte patch. Literal gameplay strings such as `name`, `dynasty`,
`monarch_names`, and `leader_names` in `history/countries` and
`common/countries` can render as mojibake when stored as ordinary UTF-8 CJK.
These are not localisation files: keep a readable UTF-8 source outside active
runtime folders, then generate the active file as BOM-free raw
EU4SpecialEscape bytes with `scripts/encode_eu4_special_gameplay.py`. Active
localisation remains UTF-8 BOM and uses the separate localisation generator.
Validators must detect escape markers before a UTF-8 fallback, compare the
complete generated byte stream with the source-derived expectation, reject raw
UTF-8 CJK in the active gameplay file, and round-trip decoded names for semantic
checks. A generic text reader is not sufficient for this exceptional surface.
...
For generated double-byte country files, use at least 24 quoted ASCII ship
names per country. "Unique" means case-insensitively unique across the complete
effective VFS, not merely inside one file: EU4 can discard a shared 32-name
pool repeated by 30 tags and report every tag below the threshold. Derive a
stable country/house stem plus fixed suffixes, then reject duplicates across
all generated countries and collisions with pinned vanilla or parent-Mod
providers.

Windows PowerShell 5.1 does not default to UTF-8 when it reads a BOM-free text
file. Every validator read of EU4 scripts, descriptors, localisation, `.gfx`,
and country-name pools must therefore specify `-Encoding UTF8` (or use an
equivalent explicit UTF-8 API). Otherwise valid Chinese names can be decoded
into whitespace or punctuation, producing false list counts and misleading
`countrydatabase` warnings. Test this behavior with a BOM-free UTF-8 fixture;
do not pad a valid name pool merely to silence a code-page-induced warning.
...
Area identity has two layers: the Clausewitz key in `map/area.txt` and the
localised display value. Unique keys do not guarantee unique names after a
translation stack is applied. EU4 1.37.5 can log `map.cpp:113` with `Two areas
have the same name` when, for example, a Japanese land area and a Chinese sea
area both resolve to the same short label. Keep custom area/region labels in a
readable localisation plan, generate the active escaped file from that plan,
and compare custom labels with the effective vanilla/translation labels used by
the supported playset. Prefer the precise historical geographic term (such as
Tokaido for the Japanese circuit) over a shortened name already used by a sea
area.
...
Split the ownership proof into two sources. Provinces without an intentional override must have the exact root owner and dated owner/controller sequence from the pinned vanilla file. Every new province and each deliberate old-province divergence must have one explicit, sorted, unique, in-campaign, non-no-op timeline. Export a CSV of `province_id, source, parent, start_inclusive, end_exclusive, owner` and verify that it reconstructs the generated histories exactly. When replacing ownership in a copied vanilla history, scrub only `owner` and `controller`; deleting `add_core` or `remove_core` while changing owners silently destroys unrelated historical state. Simulate root and dated `add_core`/`remove_core` actions in engine order across the full timeline and require zero attempts to remove an absent core. Do not "fix" a delayed vanilla core removal by rebuilding cores from owner transitions: ownership and core chronology are intentionally different. In log triage, resolve the effective provider for the reported province ID; a `province.cpp:9468` line is blocker-level when that provider is main, companion, or compatibility content, and only game-root-only providers may enter a documented vanilla-benign class.
...
When a new tag represents a historical successor that vanilla models inside the
predecessor tag, province handoff alone is incomplete. Override the predecessor
country history so its ruler and dynasty stop carrying the successor identity at
the exact handoff date; close every predecessor subject interval at that same
boundary; and give the successor its own country history, ideas, government
identity, and full non-generic mission profile. Evaluate every ownership change
boundary and its preceding day across both the standalone and companion-map
timelines. Hard-fail if both tags are landed on one selectable date, if the
successor owns land early, if the predecessor returns late, or if the successor
falls through to generic missions. Keep the readable country histories as the
generator's source of truth and generate both sides together so later timeline
rebuilds cannot restore the duplicated ruler.

Successor identity also survives outside country and ownership history. Audit
every post-handoff file in `history/wars`: replace the predecessor in
`add_attacker`, `add_defender`, `rem_attacker`, and `rem_defender`, and inspect
the `country` field on every historical battle side. Then audit dated province
control separately from ownership. An invasion normally leaves `owner` with
the invaded country and changes only `controller` for a half-open occupation
interval; validate every start, end, and preceding day as an exact province
set. Check battle `location` IDs against the pinned map as well, since an
otherwise correct war can place a named battle in the wrong province. Finally,
prove that minor co-belligerents have a valid overlord/subject state on the war
date and converge every translation or compatibility provider that owns the
same war or province-history path.

That proof must include the complete active VFS. Do not elevate one observed
`dlc_load.json` ordering into a universal winner rule: launcher order,
dependencies, and engine resolution are separate facts. Translation packages
make this especially dangerous. The 1.37 Chinese supplementary package owns
`history/countries/ODA - Oda.txt`, dozens of Japanese province histories,
Japanese country-name pools, and `missions/DOM_Japanese_Missions.txt`; its old
timeline can coexist with companion TOY provinces and produce two landed
Hideyoshi states. The robust solution is provider convergence: clone or patch
the supplementary package locally, replace every critical collision with the
authoritative gameplay/map bytes, canonicalize its literal gameplay-name bytes,
disable the divergent Workshop provider, and verify every remaining provider is
byte-identical. Then sample 1586-01-01, 1598-05-23, 1600-10-21, 1615-06-03, and
1615-06-04. Source-plan and per-mod timeline checks do not cover this failure
class.
...
When overriding a vanilla national idea group such as `ODA_ideas` or `ASA_ideas`, write the whole group: `start`, `bonus`, `trigger`, `free = yes`, and all seven ideas. For a broad EU4 1.37.5 national-idea overhaul, do not infer a safe additive-file window from lexical order. JXP tested both files before `00_basic_ideas.txt` and files apparently between `00_basic_ideas.txt` and `00_country_ideas.txt`; the latter still produced an in-campaign IdeaDatabase containing only the basic selectable groups, while the country-selection preview appeared plausible. The robust version-pinned pattern is one generated exact-path registry:

- keep readable custom idea files in a tooling/source directory outside runtime `common/ideas`;
- pin the complete vanilla `common/ideas/00_country_ideas.txt` by hash;
- parse complete top-level groups, remove each vanilla group replaced by the mod, and emit every full custom group plus all unreplaced vanilla groups to runtime `common/ideas/00_country_ideas.txt`;
- emit BOM-free gameplay text and require byte-exact generator `--check` output;
- reject every second runtime `.txt` in `common/ideas`, duplicate top-level ID, missing vanilla group, missing custom group, malformed `start + 7 ideas + bonus`, or trigger/tag mismatch;
- treat every other enabled provider of `common/ideas/00_country_ideas.txt` as a critical VFS collision: make it byte-identical through a compatibility clone or declare the playset unsupported;
- for optional companion countries, append their dormant groups to the sole parent registry and gate each with `parent-owned stage trigger + companion origin flag`; write the origin flag at the country's historical start date and keep a release/initialization fallback, while the stage gate prevents origin flags from stacking old ideas after route/final formation;
- after deployment, inspect a fresh in-campaign `setup.log`, but model the engine's observable shape rather than requiring every source label. In EU4 1.37.5, IDs `#26..#225` can be a normal silent range even though those groups exist. Prove silent groups from the byte-exact generated registry; place companion sentinels at the registry tail so they receive logged IDs, then require the exact visible ID set, unique ID/tag pairs, named tail anchors, and `IdeaDatabase Initialized`. Country-selection preview cards do not satisfy this check.

When that exact-path registry adds one collision to a compatibility clone,
require `common/ideas/00_country_ideas.txt` itself to be present in the patch
manifest and require its authoritative source to be the gameplay Mod. Increase
the expected aggregate only after a fixture supplies divergent source/main
bytes and proves the clone emits the main bytes; collision counts alone do not
identify which path was secured.

Keep matching localisation in the same version slice and regenerate active localisation immediately. A clean parser or correct preview can coexist with runtime default ideas, mixed tooltips, and `Missing Icon 'national_idea8'` when the engine registry is incomplete.

Tag changes need a separate free-idea-group transition. A country can retain its old free national ideas while the new tag's group is also selected, producing a new group title over old idea tooltips, overlapping icons, and `Missing Icon 'national_idea8'` in `error.log`. Put one guarded `swap_free_idea_group = yes` in the ordinary canonical sync, call it only when `has_custom_ideas = no` and the exact expected `has_idea_group` is absent, and set migration markers without using their absence as a swap condition. Keep forced-sync effects callable only from version-gated one-time old-save migrations. If a broken release truncated the idea registry itself, add a new migration over every managed main tag, including ordinary daimyo and final tags, so already-set route migration flags cannot strand default or stacked groups. When route identity requires the new ideas, do not use the optional `ideagroups.1` prompt: declining it preserves stale identity by design. Validate exactly seven ideas per group, one generated runtime registry, narrow ownership of every swap helper, complete tag-to-group guards, and zero route calls to `ideagroups.1`.

Historical war titles and battle labels under `history/wars` are literal `name = "..."` values, not localisation keys. A Chinese localisation file cannot translate `Imjin War` if the war history still contains that English literal. Keep readable translated literals in a generator source or mapping, encode active war files as BOM-free EU4SpecialEscape bytes, and validate exact decoded root names, every battle name, canonical bytes, participants, dates, goals, and locations together.
...
For the local `japan_expanded_v2`/`jxp_` framework, the coverage entrypoint preserves old-project compatibility while avoiding stale parallel contracts. On current layouts it detects and delegates to the mod-owned authoritative validator; only pre-consolidation layouts execute the legacy coverage checks:
...
The normal current-project release command remains:
...
Do not feed gameplay history or country-name files through the localisation
output contract. For literal names required by a double-byte patch, run:

```powershell
$py = "$env:USERPROFILE\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe"
& $py "$env:USERPROFILE\.codex\skills\eu4-modding\scripts\encode_eu4_special_gameplay.py" `
  "path\to\readable-country-source.txt" `
  "path\to\mod\history\countries\TAG - Country.txt"
```

The generated gameplay file must be BOM-free and may contain byte values that a
normal CP1252 encoder rejects inside EU4SpecialEscape triplets. Compare bytes,
not visually decoded console output, when checking generation drift.
...
Method names inside legitimate dynamic text are executable identifiers, not
display prose. Keep the full token ASCII, for example
`[Country.GetVaishyasName]影响力`; translating `Vaishyas` inside the brackets
causes `ck2text.cpp:267` unknown-property errors. If an enabled Workshop
translation contains such a defect, do not edit its subscribed files. Generate
a later-sorting local compatibility localisation file with only the affected
keys, EU4SpecialEscape-encode it with a BOM, and audit its exact bytes and key
set whenever the compatibility clone is rebuilt.
...
Disaster `on_start` and `on_end` fields are scalar event references in EU4
1.37.5. Do not place cleanup effects directly inside `on_end = { ... }`; the
event manager reads `{` as the event identifier and later disaster definitions
may fail to register. Move cleanup into a hidden triggered event and reference
its ID:

```txt
on_end = my_mod_disasters.90

country_event = {
	id = my_mod_disasters.90
	title = none
	desc = none
	picture = COURT_eventPicture
	hidden = yes
	...
	option = {
		name = "OK"
		remove_country_modifier = my_disaster_modifier
	}
}
```

Custom disaster progress has a separate load-order trap. In the pinned EU4
1.37.5 executable, placing `add_disaster_progress = { disaster = my_disaster }`
directly in `common/scripted_effects` can compile before `my_disaster` is in
the disaster registry and emit `failed effect validation`. Vanilla applies the
effect from the event layer. Keep the reusable debug setup in a scripted
effect, dispatch a complete hidden triggered event from it, and change progress
inside that event:

```txt
# common/scripted_effects
my_debug_seed_effect = {
	country_event = { id = my_mod_disasters.80 }
}

# events
country_event = {
	id = my_mod_disasters.80
	title = none
	desc = none
	picture = COURT_eventPicture
	hidden = yes
...
The Clausewitz parser cannot prove this registration order. Add a static guard
against custom disaster progress in `common/scripted_effects`, then require a
fresh engine `error.log` before claiming the debug seed works.

Hidden events still need a complete event shell: `title`, `desc`, `picture`, and
at least one `option`. Hiding the window does not waive those parser contracts.

Modifier keys are scope-specific. For EU4 1.37.5 event modifiers, use this
runtime-verified mapping instead of plausible aliases:

| Rejected in `common/event_modifiers` | Valid key for that scope |
| --- | --- |
...
Attach it to a real reform that is registered in the target government's
`reform_levels` and is valid for the countries that should own the mechanic:
...
my_identity_reform = {
	...
	potential = {
		OR = {
			my_country_trigger = yes
			has_reform = my_identity_reform
		}
	}
...
Do not create an unregistered custom `basic_reform` for this purpose. EU4
1.37.5 uses exactly one basic reform per government category
(`monarchy_mechanic`, `republic_mechanic`, `theocracy_mechanic`,
`tribe_mechanic`, and `native_basic_reform`). A second custom basic reform can
replace the category identity even when its script looks like an invisible
bonus: a monarchy may render republican tradition, direct `has_reform = daimyo`
checks may fail, and the country can fall out of its national mission profile.
If a vanilla identity reform such as `daimyo` or `shogunate` must gain an
ability, pin the complete vanilla source file, make the smallest exact-path
override, and validate that removing the added ability blocks reproduces the
pinned vanilla bytes. If full-file ownership is unacceptable, redesign the
mechanic rather than relying on an unregistered carrier.
...
After a route or tag change modifies which registered reform should carry the
mechanic, synchronize the one exact reform and rebuild the candidate cache:

```txt
add_government_reform = my_identity_reform
regenerate_government_mechanics = yes
```

Keep all direct grants in one canonical state-sync effect. A pre-final event
must not add a tier-one final reform before the corresponding tag/state exists,
because that can displace `daimyo`, `shogunate`, or another valid foundation.
For every fresh-start identity, verify the effective country history through
1444-11-11, the exact tier-one reform, the expected authority currency, all
required mechanics, mission ownership, and a fresh log. Include a negative
control such as a nearby monarchy that must retain vanilla `autocracy_reform`
and must not enter a mechanic retry loop.
...
6. If testing with Chinese localisation mods, scan their non-localisation payload. Keep a readable dependency order, but require every enabled provider of critical history, mission, and gameplay-name paths to converge byte-for-byte; do not rely on an assumed list-direction winner.
...
- Treat a mission-series ownership split as a bidirectional identity migration. One new positive anchor is insufficient because EU4 can retain a serialized old shared series or fill an exposed slot with a generic series while that anchor still succeeds. For each affected tag and DLC state, pin the one expected series key and reject every sibling exact-identity anchor, replaced shared-family anchor, and eligible generic fallback anchor. Capture stable completion depth from old mission IDs before the first swap, run the canonical immediate-plus-delayed refresh, and replay only the corresponding new missions under a one-time guard after the new series is active. Drive generated identity branches from one deterministic plan that emits missions, triggers, modifiers, migration, cleanup, and localisation together; validate exact tag potential, chapter count/order, generated-byte drift, and full effective topology.
...
When a flavor mod needs special institutions for many route tags, choose one of
two legal surfaces instead of auto-granted custom basic reforms:

- register player-facing reforms in the existing semantic tiers, narrow their
  `potential` to the exact route/state plus self-preservation, and keep direct
  grants out of ordinary route-entry events;
- represent guaranteed route bonuses with country/event/triggered modifiers,
  using one canonical route sync to remove mutually exclusive state.

Do not mark these entries `basic_reform = yes`, do not declare one hidden entry
as both monarchy and republic, and do not leave a `government_abilities` reform
outside every `reform_levels` list. If an old hidden carrier has already shipped,
retire its ID as an inert `always = no` tombstone, remove all auto-grant sites,
and move only genuinely unique rewards to a legal surface. Do not duplicate
bonuses already replaced by later visible reforms.
...
- reject `basic_reform = yes` on every custom reform, visible or hidden; it is a government-category foundation marker, not a general no-slot carrier API;
- distinguish candidate membership from selection: EU4 evaluates `potential` to build each tier, then applies `trigger` to decide what can be selected. A broad Japanese-polity `potential` across many mutually exclusive route reforms can therefore load every route into the same tier and hit the 1.37.5 hard ceiling of 16 candidates before the route `trigger` hides anything;
- choose the cache strategy by surface. For a small imperial-reform list, keep dynamic mission/emperor gates in `trigger` because late `potential` changes may not refresh. For a large route family, use an exact final-state scripted trigger in `potential`, preserve a chosen reform with `has_reform = <self>`, and call `regenerate_government_mechanics` from every route transition and old-save migration. Validate actual `potential` visibility and `trigger` selectability as separate matrices;
- include every identity that can legally retain its tag across a route choice. In JXP, TOY can choose Sakoku or Open without becoming JAP, so those six route reforms need a narrow `tag = TOY` plus unique-route-flag branch; uncommitted TOY sees none, valid Sakoku/Open TOY sees exactly its three, and a stacked/corrupt TOY state sees none;
...
- preserve hardcoded government identities when a route reform occupies the same tier. In particular, a Japanese Emperor of China must keep the exact `celestial_empire` reform because imperial reforms and vanilla Mandate logic key directly on it. Use a supported `conditional = { allow = { <stable Japanese identity> } government_abilities = { <custom mechanic> } }` block on the pinned vanilla reform, and mirror the displaced route reform's exact modifier signature through one mutually exclusive `common/triggered_modifiers` entry per final state. Gate each mirror with `is_emperor_of_china = yes`, `has_reform = celestial_empire`, and the exact state trigger; make non-EoC route reform `potential` reject the Emperor of China. Mission/capstone gates must be dual-state rather than directly requiring only the route reform, and Mandate gained/lost hooks must re-enter one canonical sync. Validate the ten-state EoC/non-EoC matrix, modifier equivalence, foreign-emperor negatives, and stale same-tier cleanup;
...
- when expanding mature route reform trees with a new optional level, validate the full chain: definition, exact candidate `potential`, selectable `trigger`, government-level registration, level localisation, reform localisation, route-switch cleanup, profile-by-profile candidate sets, and per-tier capacity. Treat `countryreformview.cpp:289 Trying to add more than 16 government reforms to one tier` as a runtime hard failure; keep the JXP-owned route/final/founder contribution at three or fewer candidates in every concrete tier so vanilla entries retain headroom;
- validate the exact modifier key/value signature of each visible reform, not merely its modifier count. A syntactically plausible key may survive static Clausewitz parsing but be ignored by the engine; for example EU4 1.37.5 uses `navy_tradition`, not `naval_tradition`. Pin corrected aliases with mutation tests and search the pinned vanilla reform/idea/modifier registries before accepting a new key;
