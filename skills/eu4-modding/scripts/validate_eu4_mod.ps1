...
function Test-NameInGfx {
	...
	foreach ($interfaceRoot in $interfaceRoots) {
		...
		foreach ($file in $gfxFiles) {
			...
			if (Select-String -LiteralPath $file.FullName -Pattern $pattern -Encoding UTF8 -Quiet) {
				return $true
			}
		}
	}
...
if (-not (Test-Path -LiteralPath $outerDescriptor)) {
	...
	$outerText = Get-Content -Raw -Encoding UTF8 -LiteralPath $outerDescriptor
...
foreach ($file in $locFiles) {
	...
	$firstLine = Get-Content -Encoding UTF8 -LiteralPath $file.FullName -TotalCount 1
...
foreach ($file in $tagFiles) {
	$matches = Select-String -LiteralPath $file.FullName -Pattern '^\s*([A-Z0-9]{3})\s*=\s*"countries/(.+?)"\s*$' -Encoding UTF8
...
foreach ($file in $missionFiles) {
	$icons = Select-String -LiteralPath $file.FullName -Pattern '^\s*icon\s*=\s*([A-Za-z0-9_]+)' -Encoding UTF8
...
foreach ($file in $eventFiles) {
	$pictures = Select-String -LiteralPath $file.FullName -Pattern '^\s*picture\s*=\s*([A-Za-z0-9_]+)' -Encoding UTF8
...
$disasterFiles = Get-ChildItem -Recurse -File -LiteralPath (Join-Path $ModPath "common\disasters") -Filter "*.txt" -ErrorAction SilentlyContinue
foreach ($file in $disasterFiles) {
	$inlineEndHooks = Select-String -LiteralPath $file.FullName -Pattern '^\s*on_end\s*=\s*\{' -Encoding UTF8
	foreach ($match in $inlineEndHooks) {
		Add-Issue "ERROR" $file.FullName.Substring($ModPath.Length + 1) "Disaster on_end must be a scalar event id, not an inline effect block (line $($match.LineNumber)). Move cleanup into a hidden triggered event and reference that event id."
	}
}

$invalidEventModifierKeys = [ordered]@{
...
$eventModifierFiles = Get-ChildItem -Recurse -File -LiteralPath (Join-Path $ModPath "common\event_modifiers") -Filter "*.txt" -ErrorAction SilentlyContinue
foreach ($file in $eventModifierFiles) {
	foreach ($invalidKey in $invalidEventModifierKeys.Keys) {
		$matches = Select-String -LiteralPath $file.FullName -Pattern "^\s*$([regex]::Escape($invalidKey))\s*=" -Encoding UTF8
		foreach ($match in $matches) {
			$replacement = $invalidEventModifierKeys[$invalidKey]
			Add-Issue "ERROR" $file.FullName.Substring($ModPath.Length + 1) "Event modifier key '$invalidKey' is rejected by EU4 1.37.5 (line $($match.LineNumber)); use '$replacement' for this scope."
		}
	}
}
